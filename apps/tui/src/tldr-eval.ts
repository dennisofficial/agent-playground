...
const JUDGE_INSTRUCTION = [
  ...
  '5. the status matches the ending — "done" only when the ask is fully answered,',
  '   "needs-operator" when the operator\u2019s reply is the next move, "waiting" when background',
  '   work (a shell, a sub-agent) is what happens next. When both hold, "needs-operator" is',
  '   correct — an unanswered ask outranks a wake that arrives on its own.',
