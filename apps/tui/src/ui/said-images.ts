 ...
 * transcript is the only place left to see what it took.
 */
export const saidImageLine = (image: SaidImage): string =>
  [`${glyph.image} ${basename(image.path)}`, sized(image), cost(image)]
