...
export function start(
  ...
  return {
    ...
    events,
    holds,
    // The harness keeps its one-argument shape and mints the id here. Every id is fresh and no test
    // asserts on it: these specs are about the HOLD, and the ack correlation has its own tests.
    steer: (text) => turn.steer({ id: randomUUID(), text }),
