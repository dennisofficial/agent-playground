...
| 2 | Add a `DriverApprovalGateway` neutral leaf module (mirroring the existing `BrainGateway` cycle-breaker pattern) exposing `resolveShip`/`retractShip`/`resolveMerge`/`neutralizeAmendProposal`; replace the 6 `ModuleRef`+lazy-`import()` lookups at `surface/web-surface.module.ts:212,226,245,246,263` and `surface/resolve-merge-approval.ts:18` | H5, L1 | low | ✓ done | Link 2 (job `08347afa`). New `@Global` `driver-approval-gateway/` leaf module (mirrors `brain-gateway/`); `DriverModule.onApplicationBootstrap` binds a driver-backed adapter; the surface injects the gateway as a typed collaborator. Deleted `resolve-merge-approval.ts`; controller `moduleRef` slot → `driverApproval`. The 7th lookup (`ProdDiagnosticsService` at `:296`) is NOT part of H5/L1 — left in place. Verified: backend `tsc --noEmit` clean, full unit suite 2654/2654; real-HTTP int `web-surface.auto-approve` (14, ship bridge→gateway→driver) + `web-surface.auto-merge` (8, `POST /approve` MERGE→`resolveMerge`→`mergeNow`) + `driver-store`/`shipping`/`host-retry-backstop` (36); and a **live** standalone boot (`:4002`, clean, no DI cycle) with a real curl `POST /approve` `atlas_approval:ship` → job flipped `awaiting_ship_review → running`, `ship_review_approved_at` stamped, `[ThreadDriver] ship approval … — re-driving to ship` logged (proving the driver handler fired through the bound gateway). |
| 3 | Register a global `APP_PIPE` `ValidationPipe({ whitelist:true, forbidNonWhitelisted:true, transform:true })` in `AppModule`; convert the un-validated interface DTOs in `surface/web-surface.controller.ts:355-468` (`CreateThreadDto`, `MessageBatchDto`, `ApproveDto`, `ProvideSecretDto`, etc.) to `class-validator`-decorated classes; drop the repeated per-controller local `ValidationPipe` instances | H6 | med | ✓ done | Link 3 (job `bdf2a841`). Global `APP_PIPE` in `AppModule` (`app.module.ts` providers); the 9 web-surface interface `@Body()` DTOs (incl. `DraftPayloadDto` from PR #298) → permissive class-validator classes — `@Allow()` on the fields the handlers coerce themselves (`messages`, `autoMerge`, `dependsOn`, `prNumber`, `items`, `stagedAnswers`/`comments`) so multipart/union parsing is byte-identical; `@IsOptional() ruledBy?` added to `ApproveDto` (accepted-but-ignored) so `forbidNonWhitelisted` doesn't 400 the frontend's every-call `ruledBy`. Removed 11 local `@UsePipes`. Interface/inline-typed bodies (test-bridge, `addJobDependency`) are Object-metatype → pipe skips them (verified vs `@nestjs/common` source). Verified: `tsc --noEmit` clean; web-surface auto-approve/auto-merge int tests green (drive real `/approve` incl. `ruledBy`); **live** boot + curl — valid `POST …/jobs` → 201, extra-field → 400 `property … should not exist`, `PATCH …/auto-merge {autoMerge:true}` → 200, `/approve` w/ `ruledBy` → handler `Unknown actionId` 400 (not a pipe 400). |
| 4 | Guard the 4 hand-managed indexes (`idx_memory_embedding_hnsw`, `idx_tickets_embedding_hnsw`, `uq_threads_job_parent_ordinal`, `uq_threads_ticket_id`) against `migration:generate` drift — either `@Index(name,{synchronize:false})` or a post-generate prune lint — per the regression already caught+reversed in `backend/migrations/1783597139527-HaltOutcome.ts` / `1783601000000-RestoreDroppedIndexes.ts` | H7 | low | ✓ moot | Link 4 (job `37654d74`). Already fully remediated on the current base — no code change. The 2 live indexes `idx_memory_embedding_hnsw` (`MemoryEntity`) + `uq_threads_job_parent_ordinal` (`ThreadEntity`) already carry the recommended `@Index(name,{synchronize:false})` guard (commit `d5577e86`); the 2 ticket indexes `idx_tickets_embedding_hnsw` + `uq_threads_ticket_id` no longer exist — the `tickets` table + `jobs.ticket_id` linkage were hard-dropped by `DropTickets1784000000000` (which itself notes it "makes ticket #9 drift moot"). **Live proof**: fresh pgvector Postgres, all migrations applied clean, then `pnpm db:migration:generate` → `No changes in database schema were found` (zero spurious `DROP INDEX`; no migration emitted); DB confirms the 2 live indexes present and `tickets`/`jobs.ticket_id` absent. |
