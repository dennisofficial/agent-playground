...
The typed terminal record (a) is rendered to a durable `.atlas/threads/<id>/completion.md` (files-as-store, like the Atlas-promoted `.atlas/adr/` ADR store, distinct from this repo's hand-authored `docs/adr/` records), and (b) is delivered to the **job brain**, which triages each piece: next-thread handoff (finally populating the dead `orientation` hook), ADR entry, operator card, or a `blocked` question that *cannot be silently filed*. This replaces the lossy 4000-char prose handoff as the source of truth while keeping thread→thread carry for the happy path. The driver stays the deterministic loop; the brain gains decision authority **at the boundaries** (it does not own control flow — see Alternatives).
