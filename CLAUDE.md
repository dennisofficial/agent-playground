...
| `worktrees/` | Employee-managed git worktrees (`WorktreeService`) — isolated full-repo checkouts at `<repoRoot>/.worktrees/<id>-<slug>` cut off the repo at `WORKER_ROOT`; git is the durable store (re-adopted on boot; shared association in `branch.<b>.agent-shared` config), mutating git ops mutex-serialized; every session requires one. Multi-employee features converge on a `shared/<slug>` integration branch (never checked out): personal branches cut from it, `publish`/`pull` merge through it (conflicts left in-progress for a session to resolve) — the shared branch is what Dennis reviews |
