...
**Canonical architecture doc:** `backend/src/app/ARCHITECTURE.md` — the job/session model (host scheduler + per-thread sessions, the pipeline, event intake, known divergences). **Read it first** when resuming Atlas work — it is REWRITTEN to the current vocabulary (Job = container; Thread group = first-class ordinal-ordered pipeline unit; Thread = ONE isolated Claude Code session with a `role`, belonging to exactly one thread group; Section = builder thread group, Leg = builder thread). There is no persistent "brain"/Main session — the host only schedules + seeds; every thread is its own fresh session. "Step" is RETIRED — the `steps` table (and `build_legs`/`codex_reviews`) are gone; see `docs/adr/0009-threads-architecture-redesign.md` for the current decision record (it supersedes `docs/adr/0008-first-class-thread-groups.md` in part). `backend/src/app/ATLAS_V2.md` is kept for deeper detail + build history but uses the OLD vocabulary (thread=container, track/section=lane, phase=step) AND predates the org→repo→job rebuild — treat it as history, not truth.
...
- A **Job** (`jobs`, real uuid id) is a conversation/work unit on a repo (owns the branch + worktree + sandbox + PR + one `messages` log, now thread-scoped via `messages.thread_id`). A Job has many **Thread groups** (`thread_groups`) — the ordinal-ordered, append-only pipeline (`planning → section×N → master_review → post_build → ship`) — and each Thread group has one or more **Threads** (`threads`), each ONE isolated Claude Code session carrying a `role` (`planner|codex_review|builder|review_agent|review_fix|master_review|post_build|ship`). A builder-kind (`section`) thread group is a **Section** (shown "Section 1/2/3" with the § glyph); a builder thread inside it is a **Leg** (Leg 1/2/3). Planning + plan-review collapse into the ONE `planning` group (a chattable `planner` thread + a read-only `codex_review` thread). The host is a pure scheduler + JIT context-seeder — there is **no persistent "brain"/Main session**; every thread is its own freshly-seeded session, and `jobs.focused_thread_id` is the server-authoritative routing pointer the console opens to. A build thread group's shared checklist lives in a thread-group-owned `tasks` table; subagents (Task-tool agents) get their own `subagents` table. NOTE: the DB table for the container is `jobs`; the pipeline-grouping table is `thread_groups`; the lane table is `threads` (formerly `tracks`, `kind` renamed to `role`); the internal gap-numbered `ordinal` is never displayed (the UI derives a per-kind Section/Leg index).
