...
export function useIdlePremeasure(opts: {
  ...
  enabled: boolean;
  chunkSize?: number;
  /** ```mermaid fence sources for rows this pass may measure (deduped by the warm helper itself). Warmed
  ...
  // `warmSources` is recomputed (new array identity) by the caller on essentially every source-list change,
  // which can happen faster than the idle warm sweep completes during a busy turn. Depending on identity
  // would tear down and reschedule the in-flight warm on every such change, potentially starving `warmed`
  // forever and blocking the whole MEASURE/SEED pass — so key the effect on CONTENT instead (like `topKey`
  ...
  useEffect(() => {
    ...
    const list = warmSourcesRef.current ?? [];
    if (list.length === 0) {
      setWarmed(true);
      return;
    }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- warmKey is a content-stable proxy for warmSources
  }, [enabled, warmed, warmKey]);
