...
import { classifyDiffLine, diffRenderer } from '../renderers/diff.js';

describe('classifyDiffLine', () => {
  it('claims file headers before it claims added and removed lines', () => {
    // The whole reason this function is ordered rather than a lookup table: `---`/`+++` start with
    // the same characters as a change, and misreading them opens every diff with a false pair.
...
describe('diffRenderer', () => {
  ...
  it('measures the block at its longest line, so the panner has a range to work with', () => {
    const view = diffRenderer.render('+short\n-a much longer line here', 'diff', 10);
