...
class CsvUserImportJob extends ImportJob<UserRow> {
...
- You have a stable, closed set of node types (an AST, a document's element types) and keep adding new _operations_ over them (print, validate, serialize) rather than new node types.
...
function toHtml(node: Node): string {
  switch (node.kind) {
    ...
    case 'link':
      return `<a href="${node.href}">${node.children.map(toHtml).join('')}</a>`;
  }
}
```
