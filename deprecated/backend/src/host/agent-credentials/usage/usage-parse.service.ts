...
const RATE_LIMIT_TYPE_TO_WINDOW: Record<string, ClaudeUsageWindowKey> = {
  five_hour: 'fiveHour',
  seven_day: 'sevenDay',
...
/** Parses Claude's `/api/oauth/usage` response (and harvested rate-limit frames) into stored windows. */
@Injectable()
export class UsageParseService {
  /** Maps a raw `rate_limit_type` (e.g. "five_hour") to its stored window key, if known. */
  windowKeyFor(rateLimitType: string | undefined): ClaudeUsageWindowKey | undefined {
    return rateLimitType ? RATE_LIMIT_TYPE_TO_WINDOW[rateLimitType] : undefined;
  }

  toPercentUtilization(utilization: number | undefined): number | undefined {
    if (utilization == null) return undefined;
    const percent = utilization <= 1 ? utilization * 100 : utilization;
    return Math.round(Math.min(100, Math.max(0, percent)));
  }

  resetEpochToIso(resetsAt: number | undefined | null): string | undefined {
  ...
  parseModelWindows(root: Record<string, unknown>): ModelUsageWindow[] {
  ...
  parseUsageResponse(body: unknown): ParsedUsage {
    ...
    return { windows, modelWindows: this.parseModelWindows(root) };
  }

  private parseWindow(raw: unknown): StoredUsageWindow | null {
