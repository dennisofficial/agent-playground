...
describe('keeping shells scoped to the thread that started them', () => {
  ...
  it('refuses to read, peek at, or kill a shell another thread started', async () => {
    ...
    expect(
      registry.kill({ shellId: theirs.snapshot.shellId, by: EKilledBy.Model, threadId: THREAD }).ok,
    ).toBe(false)
    expect(registry.read({ shellId: theirs.snapshot.shellId, threadId: ELSEWHERE }).ok).toBe(true)
  })
