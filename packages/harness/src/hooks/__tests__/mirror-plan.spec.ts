...
describe('MirrorPlanHook', () => {
  it('mirrors a written plan into context so it survives compaction', async () => {
    const outcome = await hook.run({ call: callOf({ name: PLAN_TOOL_NAME, input: PLAN }), result: OK })
  ...
  it('ignores every other tool', async () => {
    const outcome = await hook.run({ call: callOf({ name: 'read', input: PLAN }), result: OK })
