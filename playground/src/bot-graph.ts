...
/**
 * A shallow copy of `m` with a cache breakpoint on its last non-thinking content block — WITHOUT mutating
 * the original (it rides in checkpointed state, so a mutation would poison the durable history). Only
 * anchors on human/ai turns carrying cacheable text: a `tool` tail would need `cache_control` on the
 * tool_result block itself, not nested inside its content (which is what wrapping a string into a text
 * block would do), and an empty turn has nothing to anchor. Both cases return `m` unchanged, so that turn
 * falls back to system-only caching — the trailing messages get cached under the next turn's breakpoint
 * anyway. `thinking` and `redacted_thinking` blocks are intentionally skipped when searching for the anchor:
 * Anthropic rejects `cache_control` on thinking blocks, and a thinking-only assistant turn returns `m`
 * unchanged. The clone is ephemeral (only ever handed to `model.invoke`, never persisted), so any lc_kwargs
 * drift from the spread is harmless.
 */
const withCacheBreakpoint = (m: BaseMessage): BaseMessage => {
  ...
  for (let i = m.content.length - 1; i >= 0; i--) {
    if (!isThinking(m.content[i])) { anchor = i; break; }
  }
  return anchor >= 0
