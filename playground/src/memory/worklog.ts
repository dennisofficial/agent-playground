 ...
 * "what's true now"): this is "what happened, and when". Scoped by `company` (the project/workspace),
 * so a standup in one project never surfaces another's work. Plain SQL — no embeddings needed.
 */
...
export function logWork(e: Omit<WorkEntry, 'completedAt'>): void {
  getDb()
    .prepare(
      `INSERT INTO worklog (owner_bot, company, task, summary, completed_at) VALUES (?, ?, ?, ?, ?)`,
    )
    .run(e.ownerBot, e.company, e.task, e.summary, new Date().toISOString());
}
...
export function recentWork(q: RecentWorkQuery): WorkEntry[] {
  ...
  const rows = getDb()
    .prepare(
      `SELECT owner_bot, company, task, summary, completed_at FROM worklog
