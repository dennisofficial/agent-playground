...
export class SplitMessageSourceAxis1784611592558 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."thread_messages_audience_enum" AS ENUM('operator_only', 'shared')`,
    );
    await queryRunner.query(
      `ALTER TABLE "thread_messages" ADD "audience" "public"."thread_messages_audience_enum" NOT NULL DEFAULT 'shared'`,
    );
    // Preserve the old system_operator → operator-only distinction before collapsing the source values.
    await queryRunner.query(
      `UPDATE "thread_messages" SET "audience" = 'operator_only' WHERE "source" = 'system_operator'`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "thread_messages" ALTER COLUMN "source" TYPE text USING "source"::text`,
    );
    await queryRunner.query(
      `UPDATE "thread_messages" SET "source" = 'system' WHERE "source" LIKE 'system\\_%'`,
    );
    await queryRunner.query(`DROP TYPE "public"."thread_messages_source_enum"`);
    await queryRunner.query(
      `CREATE TYPE "public"."thread_messages_source_enum" AS ENUM('operator', 'atlas', 'system', 'untrusted')`,
    );
    await queryRunner.query(
      `ALTER TABLE "thread_messages" ALTER COLUMN "source" TYPE "public"."thread_messages_source_enum" USING "source"::"public"."thread_messages_source_enum"`,
    );

    // inbound_messages.source: same collapse.
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "source" TYPE text USING "source"::text`,
    );
    await queryRunner.query(
      `UPDATE "inbound_messages" SET "source" = 'system' WHERE "source" LIKE 'system\\_%'`,
    );
    await queryRunner.query(`DROP TYPE "public"."inbound_messages_source_enum"`);
    await queryRunner.query(
      `CREATE TYPE "public"."inbound_messages_source_enum" AS ENUM('operator', 'atlas', 'system', 'untrusted')`,
    );
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "source" TYPE "public"."inbound_messages_source_enum" USING "source"::"public"."inbound_messages_source_enum"`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "inbound_messages" SET "source" = 'system_shared' WHERE "source" = 'system'`,
    );
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "source" TYPE "public"."inbound_messages_source_enum" USING "source"::"public"."inbound_messages_source_enum"`,
    );
    ...
    await queryRunner.query(
      `UPDATE "thread_messages" SET "source" = 'system_operator' WHERE "source" = 'system' AND "audience" = 'operator_only'`,
    );
    await queryRunner.query(
      `UPDATE "thread_messages" SET "source" = 'system_shared' WHERE "source" = 'system'`,
    );
    await queryRunner.query(`DROP TYPE "public"."thread_messages_source_enum"`);
    await queryRunner.query(
