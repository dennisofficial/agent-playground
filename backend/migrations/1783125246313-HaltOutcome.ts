...
export class HaltOutcome1783125246313 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "threads" ADD "halt_waked_at" TIMESTAMP WITH TIME ZONE`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "threads" DROP COLUMN "halt_fix_attempts"`,
    );
    await queryRunner.query(
      `ALTER TABLE "threads" DROP COLUMN "halt_waked_at"`,
    );
