...
export class AddThreadCondition1783600100000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "threads" SET condition = 'paused'     WHERE status = 'awaiting_approval'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET condition = 'incomplete' WHERE status = 'incomplete'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET condition = 'failed'     WHERE status = 'failed'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET condition = 'skipped'    WHERE status = 'skipped'`,
    );
    // ...then collapse the removed status values onto their linear step.
    await queryRunner.query(
      `UPDATE "threads" SET status = 'executing' WHERE status IN ('awaiting_input', 'incomplete', 'failed')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "threads" SET status = 'failed'           WHERE condition = 'failed'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET status = 'incomplete'       WHERE condition = 'incomplete'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET status = 'awaiting_input'   WHERE condition = 'paused' AND status = 'executing'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET status = 'awaiting_approval' WHERE condition = 'paused' AND status = 'planning'`,
    );
    await queryRunner.query(
      `UPDATE "threads" SET status = 'skipped'          WHERE condition = 'skipped' AND status = 'reviewing'`,
    );
