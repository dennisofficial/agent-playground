...
export class AddJobActivity1783600200000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "activity" text NOT NULL DEFAULT 'idle'`,
    );
    ...
    await queryRunner.query(
      `UPDATE "jobs" SET "activity" = 'turn' WHERE "turn_active" = true`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "turn_active" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "review_running" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `UPDATE "jobs" SET "turn_active" = ("activity" = 'turn')`,
    );
    await queryRunner.query(
      `UPDATE "jobs" SET "review_running" = ("activity" = 'plan_review')`,
    );
