...
export class ConsumptionModelAndActiveTurns1784771257024 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."inbound_messages_status_enum" RENAME TO "inbound_messages_status_enum_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."inbound_messages_status_enum" AS ENUM('draft', 'pending', 'consumed', 'delivered')`,
    );
    await queryRunner.query(`ALTER TABLE "inbound_messages" ALTER COLUMN "status" DROP DEFAULT`);
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "status" TYPE "public"."inbound_messages_status_enum" USING "status"::"text"::"public"."inbound_messages_status_enum"`,
    );
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "status" SET DEFAULT 'pending'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "active_turns" DROP CONSTRAINT "fk_active_turns_job_id_jobs"`,
    );
    await queryRunner.query(
      `ALTER TABLE "active_turns" DROP CONSTRAINT "fk_active_turns_org_id_organizations"`,
    );
    await queryRunner.query(`DROP INDEX "public"."idx_inbound_messages_job_id_status"`);
    await queryRunner.query(
      `CREATE TYPE "public"."inbound_messages_status_enum_old" AS ENUM('delivered', 'draft', 'pending')`,
    );
    await queryRunner.query(`ALTER TABLE "inbound_messages" ALTER COLUMN "status" DROP DEFAULT`);
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "status" TYPE "public"."inbound_messages_status_enum_old" USING "status"::"text"::"public"."inbound_messages_status_enum_old"`,
    );
    await queryRunner.query(
      `ALTER TABLE "inbound_messages" ALTER COLUMN "status" SET DEFAULT 'pending'`,
    );
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."inbound_messages_status_enum_old" RENAME TO "inbound_messages_status_enum"`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_inbound_messages_job_id_status" ON "inbound_messages" ("job_id", "status") `,
    );
