...
export class LinkThreadsToTickets1782508269876 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "threads" ADD CONSTRAINT "fk_threads_ticket_id_tickets" FOREIGN KEY ("ticket_id") REFERENCES "tickets"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "uq_threads_ticket_id" ON "threads" ("ticket_id") WHERE "ticket_id" IS NOT NULL`,
    );
  }
