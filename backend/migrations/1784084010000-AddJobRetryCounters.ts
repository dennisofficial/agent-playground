...
export class AddJobRetryCounters1784084010000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "benign_abort_redrives" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "transient_retry_redrives" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "auth_retry_attempts" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "driver_transient_retries" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE "jobs" ADD "retry_last_attempt_at" TIMESTAMP WITH TIME ZONE`,
    );
  }
