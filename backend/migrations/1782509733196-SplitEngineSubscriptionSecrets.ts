...
export class SplitEngineSubscriptionSecrets1782509733196 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`UPDATE "org_credentials" SET "claude_oauth_token_enc" = "engine_auth_secret_enc" WHERE "engine_auth_secret_enc" IS NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "org_credentials" ADD "engine_auth_mode" text NOT NULL DEFAULT 'api_key'`);
        // Restore the single secret from the Claude column (the only one the old shape could represent).
        await queryRunner.query(`UPDATE "org_credentials" SET "engine_auth_secret_enc" = "claude_oauth_token_enc", "engine_auth_mode" = 'subscription' WHERE "claude_oauth_token_enc" IS NOT NULL`);
