...
export class SandboxGroup {
  ...
  transcriptAccess(): string {
    return [
      ...
      '    A subagent transcript shows the session that spawned it in the PARENT column.',
      '  - `atlas-tx show <sessionId> [--role assistant|user] [--thinking] [--text] [--tools] [--errors] [--subagents] [--tail N]`',
      "    — a readable, token-frugal filtered view. `--thinking` surfaces the builder's real reasoning; `--errors`",
      '    catches API-level failed tool results ONLY — an empty `--errors` is NOT proof the run was clean, since a',
      "    subagent's activity lives in its OWN transcript. `--subagents` also renders this session's subagent",
      '    transcripts (same filters), so `--errors --subagents` reaches failures a subagent hit.',
