...
export class MetaGroup {
  ...
  planReview(): string {
    return [
      ...
      "burns the author's time and devalues your review. This is a REVIEW ONLY: do NOT create, edit, or delete",
      'any file — this review often runs behind a write-guard that auto-declines a file-mutation attempt at the',
      'sandbox boundary, but never rely on that as a backstop; the no-write rule applies whether or not one is',
      'active. That guard does NOT extend to the shell: you still MUST NOT run a command that mutates the repo',
      '(no commit, no push, no deleting/moving files, nothing that changes repo state) — nothing declines that',
      'for you, so it is a hard rule on trust alone. Read and verify only — you HAVE network + live web search, and you',
