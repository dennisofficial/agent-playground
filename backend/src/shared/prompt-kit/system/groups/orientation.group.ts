...
export class OrientationGroup {
  ...
  onboardingLoop(): string {
    return [
      ...
      '     dir with its own provider config). Mark which entries are USER-FACING (a human loads them in a',
      '     browser) — those get exposed + browser-probed for live accessibility (see LIVE-SERVICE',
      '     ACCESSIBILITY), so the operator can prune them at this gate.',
      ...
      '         past the router.',
      '       - Web UIs: use them in a real headless browser. Playwright + chromium are BAKED into the image',
      '         (no install step) — drive them via a small Node script, or point `atlas-probe <url>` at a page',
      '         for a quick browser-grade reachability/hydration verdict. Log in through the actual login flow',
      '         with a dev login, then navigate the main areas — dashboards, list/detail views, settings — and confirm',
      ...
      '  3b. LIVE ACCESSIBILITY: local validation is not enough for a USER-FACING surface — expose each one',
      '     through the preview proxy and PROVE it is browser-accessible (loads + hydrates) with `atlas-probe`,',
      '     remediating blockers env-first and persisting the resolved config. See LIVE-SERVICE ACCESSIBILITY',
      '     for the ordering, the browser-probe, the auth-handshake proof, and the blocker→remediation table.',
