...
export class PlanningGroup {
  ...
  submitPlanDetail(): string {
    return [
      ...
      '  - rename: OPTIONAL boolean — whether to re-title the job from `goal`. DEFAULT is to KEEP the current',
      '    title (shown in your orientation block as the `Title:` line). Pass `rename: true` ONLY when the',
      "    plan's subject has materially DRIFTED from — or is meaningfully CRISPER than — that current title;",
      '    most plans do NOT change direction, so most proposals should omit it. You never write the name',
      '    yourself — when rename is true the short title is generated from your `goal` by the titler.',
      "  - threads: the ordered build threads (lanes), each `{ title, type }`. `type` = the thread's scope — one of the",
      '    CLOSED set backend | frontend | docs | testing | infra | data | general (data = DB/migration/schema changes;',
      '    general = the fallback when none fit); it SELECTS the review agents.',
      ...
      'could demo or review on its own, and its `type` (backend | frontend | docs | testing | infra | data | general) selects',
      ...
      'follow-up work and verify it (typecheck/build/tests + a live run of any runtime surface you touched).',
      'When it is done the job is re-parked DIRECTLY at the ship-review gate (amending → ready-to-ship, no',
      'rebuild) and the "Ship it" card is re-posted. Do not re-propose amending unless something material changed.',
    ].join('\n');
  }
