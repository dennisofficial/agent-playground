...
export class OAuthDeviceStore {
  ...
  async stash(state: CodexDeviceState, ttlSeconds: number): Promise<string> {
    ...
    await this.redis.set(
      OAuthDeviceStore.deviceKey(handle),
  ...
  async get(handle: string, orgId: string): Promise<CodexDeviceState | null> {
    const raw = await this.redis.get(OAuthDeviceStore.deviceKey(handle));
    if (!raw) return null;
  ...
  async remove(handle: string): Promise<void> {
    await this.redis.del(OAuthDeviceStore.deviceKey(handle));
  }

  private static deviceKey(handle: string): string {
