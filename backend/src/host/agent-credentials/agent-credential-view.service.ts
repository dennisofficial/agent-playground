...
export class AgentCredentialViewService {
  project(input: AgentCredentialViewInput): AgentCredentialView {
    return {
      ...
      plan: this.planLabel(input.subscriptionType),
      status: input.status,
      selected: input.selected,
      expiresAt: this.iso(input.expiresAt),
      usage: this.snapshotToUsage(input.usageSnapshot),
      createdAt: this.iso(input.createdAt) ?? new Date(0).toISOString(),
    };
  }
  ...
  private iso(d: Date | string | null): string | null {
