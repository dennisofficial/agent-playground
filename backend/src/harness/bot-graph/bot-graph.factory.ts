...
export class BotGraphFactory {
  ...
  constructor(
    ...
    // Same posture: appended last + optional so positional spec construction is untouched. DI always
    // provides it in the app; absent → the `tool_loop_guard` node is inert (returns `pass`).
