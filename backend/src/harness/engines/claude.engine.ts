 ...
 * project root, and bash commands must clear the deny-list. With `permissionMode: 'default'`, every
 * non-auto-approved tool call routes here — a programmatic gate (no interactive surface), so it
 * never blocks waiting on a human. When `planning` is set, the gate is also READ-ONLY: Write/Edit
 * are denied outright and mutating shell is refused, so a PLAN pass physically cannot touch the
 * project (the core "plan first, build only once approved" guarantee).
 */
...
export class ClaudeEngine implements WorkerEngine {
  ...
  constructor(
    @Inject(ANTHROPIC_AGENT_SDK) private readonly sdk: typeof import('@anthropic-ai/claude-agent-sdk'),
    private readonly env: EnvService,
  ) {}

  async run({ task, cwd, systemPrompt, sessionId, model, effort, planning, onEvent, signal }: RunWorkerArgs) {
    ...
    for await (const message of this.sdk.query({ prompt: task, options })) {
      if (message.type === 'system' && message.subtype === 'init') {
        ...
        for (const block of message.message.content as Array<{ type: string; text?: string; name?: string }>) {
          if (block.type === 'text' && block.text) onEvent({ kind: 'text', text: block.text });
          else if (block.type === 'tool_use' && block.name) onEvent({ kind: 'tool', name: block.name });
        }
