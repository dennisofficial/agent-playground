...
describe('ProjectsModule stores (live Postgres)', () => {
  ...
  beforeAll(async () => {
    ...
    priorDefault = (await tokens.listMeta()).find((m) => m.isDefault)?.name;
  });

  afterAll(async () => {
    if (priorDefault) await tokens.setDefault(priorDefault).catch(() => {});
  ...
  it('creates, lists, gets, and patches projects; duplicate ids conflict', async () => {
    ...
    await expect(
      projects.create({ projectId: `${P}-app`, displayName: 'dup', gitUrl: 'https://github.com/x/y' }),
    ).rejects.toThrow(/already exists/);

    const patched = await projects.update(`${P}-app`, {
    ...
    expect((await projects.get(`${P}-app`))?.displayName).toBe('The App v2');
    expect((await projects.list()).some((p) => p.projectId === `${P}-app`)).toBe(true);
  });
  ...
  it('stores tokens encrypted at rest, write-only; the first token in an empty store becomes default', async () => {
    ...
    const wasEmpty = (await tokens.listMeta()).length === 0;
    const meta = await tokens.put(`${P}-tok-a`, 'ghp_plaintext_a');
    ...
    const metas = await tokens.listMeta();
    ...
    expect((await tokens.resolve(`${P}-tok-a`))?.token).toBe('ghp_plaintext_a');
  });

  it('swaps the default atomically and resolves named vs default correctly', async () => {
    await tokens.put(`${P}-tok-b`, 'ghp_plaintext_b', true);
    const metas = await tokens.listMeta();
    const defaults = metas.filter((m) => m.isDefault);
    expect(defaults).toHaveLength(1);
    expect(defaults[0].name).toBe(`${P}-tok-b`);

    expect((await tokens.resolve())?.name).toBe(`${P}-tok-b`); // default
    expect((await tokens.resolve(`${P}-tok-a`))?.name).toBe(`${P}-tok-a`); // named override
    expect(await tokens.resolve(`${P}-tok-missing`)).toBeUndefined();

    await tokens.setDefault(`${P}-tok-a`);
    expect((await tokens.resolve())?.name).toBe(`${P}-tok-a`);
    await expect(tokens.setDefault(`${P}-nope`)).rejects.toThrow(/No token named/);
  });
  ...
  it('refuses deleting a token a project references; deletes freely otherwise', async () => {
    ...
    await expect(tokens.delete(`${P}-tok-b`)).rejects.toThrow(/referenced by 1 project/);
    await projects.update(`${P}-app2`, { tokenName: null });
    await expect(tokens.delete(`${P}-tok-b`)).resolves.toBeUndefined();
    expect((await tokens.listMeta()).some((m) => m.name === `${P}-tok-b`)).toBe(false);
  });

  it('updates a token value in place via put (upsert keeps default flag)', async () => {
    await tokens.put(`${P}-tok-a`, 'ghp_rotated');
    expect((await tokens.resolve(`${P}-tok-a`))?.token).toBe('ghp_rotated');
    expect((await tokens.listMeta()).filter((m) => m.isDefault)).toHaveLength(1);
  });
});
