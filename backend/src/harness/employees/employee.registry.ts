...
const ENGINE_NAMES: ReadonlyArray<WorkerEngineName> = ['claude', 'codex', 'langgraph'];
...
const ENGINE_MODEL_TIERS: Record<WorkerEngineName, { plan: ResolvedWorkerModel; exec: ResolvedWorkerModel }> = {
  claude: {
...
export class EmployeeRegistry implements OnModuleInit {
  ...
  onModuleInit() {
    const found = collectDecorated<EmployeeDefinition>(this.discovery, AI_EMPLOYEE_METADATA);
    ...
    for (const e of roster) {
      ...
      if (!ENGINE_NAMES.includes(e.engine)) {
        throw new Error(`Employee '${e.id}' declares unknown engine '${e.engine as string}'`);
      }
    }
  ...
  fallbackOwner(): EmployeeDefinition {
    if (!this.roster.length) throw new Error('Empty roster — no fallback owner');
  ...
  mentionedBots(text: string): EmployeeDefinition[] {
    ...
    return this.roster.filter((b) => handles.has(b.name.toLowerCase()) || handles.has(b.id));
  }
   ...
   */
  resolveWorkerModel(employee: EmployeeDefinition, mode: WorkerMode): ResolvedWorkerModel {
