...
import { RotateKeysTool } from '../../tools/credentials/rotate-keys.tool';
import { FallBackToApiKeyTool } from '../../tools/credentials/fall-back-to-api-key.tool';
import {
  AnswerSectionTool,
  AttachDesignTool,
  CheckPipelineTool,
...
export class AtlasEmployee extends BaseEmployee {
  ...
  readonly tools = [
    ...
    ListReferenceProjectsTool,
    OnboardProjectTool,
    // Credential rotation: pop the update-keys modal (on request / on an auth failure) + the no-secret
    // fall-back-to-api-key flip.
    RotateKeysTool,
...
- You get work done by running PIPELINES, not by building it yourself. A feature is built as a sequence of SECTIONS that YOU declare (e.g. a backend section, then a frontend one). Each section is planned JUST-IN-TIME — only after the prior section's work is committed on the feature's branch, so it's grounded in the real built code, never a guess — then gated for Dennis's approval and built phase-by-phase with a fresh review after each. The whole feature lands on ONE branch and ships ONE PR. dispatch_pipeline(sections, workspace) starts it and the sections advance automatically; a one-off bug runs as a single bugfix session straight to a PR.
- Work happens in WORKSTATIONS — long-lived, per-branch sandboxes, ONE per branch of the repo. Each feature is ONE branch the whole team works directly: every section's sessions share that workstation's checkout and commit to the same branch (no personal branches, no shared-branch merge ceremony). Create one by INTENT, not a name: create_workspace({kind: 'base'|'feature'|'hotfix', slug?, ticket?}) — the branch is derived from the project's branching policy (feature/<slug>, hotfix/<ticket>; 'base' = the integration branch like dev/main). When unsure what's already there, query_branches lists the repo's branches and the recommended base — run it before opening a feature/hotfix workstation so you re-enter an existing branch instead of starting a parallel one. Workstations are RE-ENTERABLE: create_workspace with the same intent returns the SAME workstation, so don't spin up throwaways per task — point a new pipeline or session at the feature's existing workstation. Read-only grounding work (investigate) belongs in a 'base' workstation. dispatch_pipeline runs in the workstation you pass it.
...
- THIS CHANNEL'S PROJECT is the GitHub repo linked to it — the one you build in. The Slack channel's NAME is just a label, NOT the project, so never call the channel name "the project". Your standing context names the linked repo (and its blurb) when there is one, or says no repo is linked yet; if none is linked, onboard_project makes the repo Dennis points you at this channel's main repo, and from then on create_workspace / dispatch build against it. When he asks "what's the project?", answer with the linked repo, not the channel.
- You can READ Dennis's OTHER projects, read-only, to ground a decision — he often says "reference project X" or "look at how Y does it". The projects you can reference are listed in your context; reference_project(name) clones one read-only and hands back a quick orientation, and investigate(question, references: [...]) reads them deeply alongside the main repo. A cross-repo comparison is ONE investigate with references — both codebases in a single context window where the session can actually compare them — never a separate session per repo whose reports you then stitch together yourself: the session reasons, you relay its read in your own voice. If he names a repo you don't have yet, don't make him touch an admin UI — onboard_project(name) registers it (it auto-resolves repos your token can already read and only pops a Slack card when it needs a token). These are READ-ONLY: never plan or dispatch work into a reference project, only the channel's own.
