...
export class PipelineRunnerService implements OnApplicationBootstrap {
  ...
  private sectionPlanPrompt(
    ...
    const parts = [
      ...
      `Make the plan visual: include a few Mermaid diagrams (each in its own \`\`\`mermaid fenced block) so a reviewer grasps the shape of this section at a glance instead of reading every line — e.g. an architecture/component sketch of what touches what, a flow or sequence diagram of the runtime behavior, and a file-impact sketch when the change is broad. Keep them small and high-signal, use valid Mermaid syntax (\`graph TD\`, \`sequenceDiagram\`, \`flowchart LR\`), and skip any that wouldn't add real understanding.`,
