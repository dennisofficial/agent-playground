...
/** Out of `compose`: revision loop, tool calls, or done. */
export const afterCompose = (
  state: BotStateType,
): 'tools' | 'compose' | 'reconcile' => {
  ...
  // that to `tools` would re-execute stale calls). composeNode clears `draft` on every
  // non-suppression return, so this route is reachable only from an actual suppression.
  if (state.draft) return 'compose'; // read-the-room revision pass
 ...
 *
 * After tools run, END the turn (skip the loop back to `compose` that would otherwise force a
 * chatty text-only follow-up) when EVERY call in the triggering message is terminal AND all
 * succeeded. `every` (not `some`) is load-bearing: a message mixing a terminal tool with an
 * informational or fallible one loops back so that result is relayed. If any terminal call has
 * status:'error' we also loop back — a failed terminal tool must not silently end the turn; the
 * model needs a chance to react to the error ToolMessage.
 */
export const makeAfterTools =
  (terminal: Set<string>) =>
  (state: BotStateType): 'compose' | 'reconcile' => {
    ...
    if (!allTerminal) return 'compose';
    // Route back to compose when any terminal call failed — scan for ToolMessages whose call_id
    ...
    return anyFailed ? 'compose' : 'reconcile';
  };
