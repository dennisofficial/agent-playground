...
export function boardEventRelayPrompt(event: BoardEvent): string | null {
  switch (event.kind) {
    ...
    // ticket-approved (PipelineRunnerService.onBoardEvent owns the plan-gate resume) and plan-attached
    // are NOT narrated here — return null so the conductor never double-handles them.
    default:
      return null;
  }
}
