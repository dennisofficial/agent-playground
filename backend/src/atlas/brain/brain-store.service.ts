 ...
 * locked plan (decision record + job + section rows) on the 'atlas' connection. Keeps the
...
export class BrainStoreService {
   ...
   * `EventStimulus` doesn't carry the id). Reads the `atlas_stimuli` row's `thread_id`. Null if the
...
/** Map an `AtlasJob` row to the in-memory `Job` shape. */
