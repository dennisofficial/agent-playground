...
function makeUser(over: Partial<AtlasUser> = {}): AtlasUser {
  return {
    id: 'u1',
    email: 'a@b.com',
...
describe('AuthService', () => {
  let users: { findOne: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn>; save: ReturnType<typeof vi.fn> };
  let jwt: { signAccessToken: ReturnType<typeof vi.fn>; signRefreshToken: ReturnType<typeof vi.fn>; verifyRefreshToken: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    service = new AuthService(
      users as unknown as Repository<AtlasUser>,
      jwt as never,
      env as never,
    );
  });
  ...
  describe('login', () => {
    it('rejects unknown email with 401 (no enumeration)', async () => {
      users.findOne.mockResolvedValue(null);
      await expect(service.login('x@y.com', 'whatever', res as unknown as Response)).rejects.toBeInstanceOf(
    ...
    it('rejects a wrong password with 401', async () => {
      ...
      await expect(service.login('a@b.com', 'wrong', res as unknown as Response)).rejects.toBeInstanceOf(
        UnauthorizedException,
      );
