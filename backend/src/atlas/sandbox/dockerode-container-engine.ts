 ...
 * API. Connects to the host daemon over its unix socket (ATLAS_DOCKER_SOCKET_PATH ?? DOCKER_SOCKET_PATH,
...
export class DockerodeContainerEngine implements ContainerEngine {
  ...
  constructor(private readonly env: EnvService) {
    const socketPath =
      this.env.get('ATLAS_DOCKER_SOCKET_PATH') ?? this.env.get('DOCKER_SOCKET_PATH');
