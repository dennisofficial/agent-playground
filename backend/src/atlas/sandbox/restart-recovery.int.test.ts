...
import type { AtlasPhase } from '../persistence/entities';
...
function makeDurablePhaseRepo(initial: { id: string; session_id: string | null }) {
  ...
  const repo = {
    findOne: vi.fn(async () => ({ session_id: row.session_id } as AtlasPhase)),
  ...
  } as unknown as Repository<AtlasPhase>;
  return { repo, row };
}
...
describe('Docker restart recovery + durable session (integration, needs Docker)', () => {
  ...
  beforeAll(() => {
    ...
    manager1 = new SandboxManager(engine1, builder1, env({ ATLAS_AGENT_HOME_ROOT: homeRoot }));
  });
