...
export class SandboxRefsService {
  ...
  /** The reference-library root on the host (ATLAS_REFS_ROOT ?? REFS_ROOT); undefined → /refs disabled. */
  refsRoot(): string | undefined {
    return this.env.get('ATLAS_REFS_ROOT') ?? this.env.get('REFS_ROOT');
  }
  ...
  async ensureReference(input: {
    ...
    if (existsSync(join(dest, '.git'))) {
      await execFileAsync('git', ['-C', dest, 'fetch', '--all', '--prune'], { env });
    } else {
