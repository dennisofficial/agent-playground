 ...
 * build when the tag already exists locally, unless `ATLAS_SANDBOX_REBUILD` is set. The image tag comes
...
export class SandboxImageBuilder {
  ...
  imageTag(): string {
    ...
    return this.env.get('ATLAS_SANDBOX_IMAGE') ?? 'atlas-sandbox:latest';
  }
  ...
  /** Ensure the base image exists locally; build it if missing (or if `ATLAS_SANDBOX_REBUILD`). */
  async ensureImage(): Promise<string> {
    const tag = this.imageTag();
    const rebuild = !!this.env.get('ATLAS_SANDBOX_REBUILD');
