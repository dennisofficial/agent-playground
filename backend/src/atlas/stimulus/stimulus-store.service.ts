 ...
 *
 *  - `seedEventThread` — an `EventStimulus` OPENS a new `atlas_threads` row (origin 'event') in the
 *    routed project's channel, persists the originating `atlas_messages` row (the notification body)
 *    AND the `atlas_stimuli` event row. The partial-unique index on (team, project, source,
 ...
 *    `atlas_messages` row + the `atlas_stimuli` chat row (no new thread, no dedupe).
 *
...
export class StimulusStoreService {
  ...
  constructor(
    ...
    @InjectRepository(AtlasStimulus, ATLAS_CONNECTION)
    private readonly stimuli: Repository<AtlasStimulus>,
  ) {}
