...
export interface EngineCoreConfig {
  /** Root for the isolated agent home (ATLAS_AGENT_HOME_ROOT ?? AGENT_HOME_ROOT). */
  homeRoot?: string;
  /** Default auth mode when a run doesn't pass explicit `auth` (ATLAS_ENGINE_AUTH_MODE). */
  authMode?: 'api_key' | 'subscription';
  /** Subscription OAuth token for Claude (ATLAS_CLAUDE_OAUTH_TOKEN). */
  claudeOauthToken?: string;
  /** Fallback Anthropic API key (ANTHROPIC_API_KEY). */
  anthropicApiKey?: string;
  /** Default Claude model (ATLAS_WORKER_MODEL ?? WORKER_MODEL). */
  workerModel?: string;
  /** Default Codex model (ATLAS_CODEX_MODEL ?? CODEX_MODEL). */
  codexModel?: string;
}
