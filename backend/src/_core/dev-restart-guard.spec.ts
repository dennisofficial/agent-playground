...
describe('checkRestartLoop', () => {
  ...
  it('a single boot with no history is not a storm', () => {
    ...
    expect(result).toEqual({ recentCount: 1, looksLikeAStorm: false, windowMs: DEFAULT_RESTART_WINDOW_MS });
  ...
  it('boots spaced well beyond the window never trip the threshold', () => {
    ...
    for (let i = 0; i < 8; i++) {
      last = checkRestartLoop({ filePath, windowMs: 30_000, threshold: 5, now: () => i * spacingMs });
    }
  ...
  it('a burst of boots inside the window trips the threshold', () => {
    ...
    for (const t of times) {
      last = checkRestartLoop({ filePath, windowMs: 30_000, threshold: 5, now: () => t });
    }
  ...
  it('boots outside the window age out of the recent count', () => {
    checkRestartLoop({ filePath, windowMs: 30_000, threshold: 5, now: () => 0 });
    checkRestartLoop({ filePath, windowMs: 30_000, threshold: 5, now: () => 1_000 });
    checkRestartLoop({ filePath, windowMs: 30_000, threshold: 5, now: () => 2_000 });
    // Jump far past the window — only this boot itself should count as "recent".
    const result = checkRestartLoop({ filePath, windowMs: 30_000, threshold: 5, now: () => 100_000 });
  ...
  it('trims stored history to `keep` entries (bounded file size)', () => {
    ...
    const result = checkRestartLoop({ filePath, keep: 5, now: () => 3_000_000 });
    expect(result.recentCount).toBe(1);
  });
...
describe('renderRestartStormWarning', () => {
  it('names the recent count and window so the message is actionable', () => {
    const msg = renderRestartStormWarning({ recentCount: 6, looksLikeAStorm: true, windowMs: 30_000 });
