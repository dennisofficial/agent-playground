...
describe('assertSingleWriteStatement', () => {
  it('accepts a plain INSERT', () => {
    expect(assertSingleWriteStatement('INSERT INTO x VALUES (1)')).toBe(
      'INSERT INTO x VALUES (1)',
    );
  });

  it('accepts a plain UPDATE', () => {
    expect(assertSingleWriteStatement('UPDATE x SET a=1')).toBe(
      'UPDATE x SET a=1',
    );
  });

  it('accepts a plain DELETE', () => {
    expect(assertSingleWriteStatement('DELETE FROM x WHERE id=1')).toBe(
  ...
  it('rejects a multi-statement write', () => {
    expect(() =>
      assertSingleWriteStatement('update a set x=1; update b set x=1'),
