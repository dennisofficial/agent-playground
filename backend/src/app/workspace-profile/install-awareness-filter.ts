...
const SYSTEM = [
  ...
  '',
  'Bias HARD to suppress: most installs add nothing worth interrupting the agent for. Suppress',
  '(`suppress: true`) when the tool is a transient/one-off run, a transitive sub-dependency, already',
  "covered by the current workspace profile below (an installed skill/MCP/setup already handles it), or a",
  'workaround for a harness/image BUG — a harness/image bug is NEVER profile material; it belongs at the',
  'image level, not persisted into the profile.',
  '',
  'Do NOT suppress a genuinely new stack or tool that would benefit from a complementary skill, an MCP',
  'server, a validation step, or persistence across sandbox resets. When you keep it (`suppress: false`),',
  'give exactly ONE specific, actionable suggestion in `suggestion` — name the skill or MCP server if you',
  'can identify one; leave `suggestion` as "" if you have nothing concrete to add beyond the generic',
  'checklist the agent already saw.',
  '',
