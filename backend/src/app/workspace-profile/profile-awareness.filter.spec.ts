...
import type { WorkspaceProfileService, WorkspaceProfileSnapshot } from './workspace-profile.service';
...
/** A fake `WorkspaceProfileService` — always returns the same trivial snapshot/render. */
function fakeProfile(): WorkspaceProfileService {
  return {
    describe: vi.fn().mockResolvedValue(EMPTY_SNAPSHOT),
...
function fakeFilter(opts: { verdict?: InstallFilterVerdict; throws?: boolean }): InstallAwarenessFilter {
  return {
    filter: vi.fn().mockImplementation(async () => {
...
describe('ProfileAwarenessService — Stage-2 filter wiring', () => {
  it('a fired transition + filter {suppress:true} → handle returns null (suppressed)', async () => {
    const filter = fakeFilter({ verdict: { suppress: true, suggestion: '', reason: 'noise' } });
  ...
  it('a fired transition + filter {suppress:false, suggestion} → Stage-1 text + the suggestion', async () => {
    const filter = fakeFilter({
      verdict: { suppress: false, suggestion: 'Add the eslint-review skill', reason: 'new lint tool' },
    });
  ...
  it('a fired transition + filter {suppress:false, suggestion:""} → plain Stage-1 text, no "Suggestion:" line', async () => {
    const filter = fakeFilter({ verdict: { suppress: false, suggestion: '', reason: 'nothing specific' } });
    const svc = new ProfileAwarenessService(fakeConfigStore('add'), fakeProfile(), filter);
  ...
  it('profile service absent → Stage 2 stays off even with a filter bound', async () => {
    const filter = fakeFilter({ verdict: { suppress: true, suggestion: '', reason: 'would suppress' } });
  ...
  it('no transition (steady-state repeat) → null, filter never consulted', async () => {
    const filter = fakeFilter({ verdict: { suppress: false, suggestion: 'ignored', reason: 'n/a' } });
  ...
  it('a "remove" transition + suppress:false → the retire checklist text, unaffected by filter shape', async () => {
    const filter = fakeFilter({ verdict: { suppress: false, suggestion: '', reason: 'genuine retire' } });
    const svc = new ProfileAwarenessService(fakeConfigStore('remove'), fakeProfile(), filter);

    const text = await svc.handle({ ...HANDLE_INPUT, command: 'pnpm remove eslint' });
