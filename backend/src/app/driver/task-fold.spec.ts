...
describe('foldTaskEvent', () => {
  it('registers a task from TaskCreate, parsing the id out of the SDK’s STRING result', () => {
    ...
    expect(tasks).toEqual([{ id: '6', subject: 'Write the migration', status: 'pending' }]);
  });

  it('registers a task from TaskCreate with a structured { task: { id } } result (fallback shape)', () => {
    const tasks = foldTaskEvent([], 'taskcreate', { subject: 'Write the migration' }, { task: { id: 't1' } });
    expect(tasks).toEqual([{ id: 't1', subject: 'Write the migration', status: 'pending' }]);
  });
  ...
  it('captures description + activeForm on create and keeps them across updates', () => {
    const created = foldTaskEvent(
      [],
      'taskcreate',
      { subject: 'Resolve chain', description: 'Config → detect → fallback.', activeForm: 'Resolving the chain' },
    ...
    const started = foldTaskEvent(created, 'taskupdate', { taskId: '2', status: 'in_progress' }, {});
    expect(started[0]).toMatchObject({ status: 'in_progress', description: 'Config → detect → fallback.', activeForm: 'Resolving the chain' });
    const edited = foldTaskEvent(started, 'taskupdate', { taskId: '2', description: 'New detail.' }, {});
    expect(edited[0]).toMatchObject({ description: 'New detail.', activeForm: 'Resolving the chain' });
  });

  it('falls back to input.description, then the id itself, when subject is missing', () => {
    const byDescription = foldTaskEvent([], 'taskcreate', { description: 'Run tests' }, { task: { id: 't1' } });
  ...
  it('is a no-op if the create result has not landed / carries no recognizable id', () => {
    ...
    expect(foldTaskEvent([], 'taskcreate', { subject: 'x' }, 'Something went wrong')).toEqual([]);
    expect(foldTaskEvent([], 'taskcreate', { subject: 'x' }, 'Task creation failed')).toEqual([]);
  });

  it('transitions status via TaskUpdate, keyed by input.taskId', () => {
    const seeded = [{ id: 't1', subject: 'Write the migration', status: 'pending' as const }];
    const tasks = foldTaskEvent(seeded, 'taskupdate', { taskId: 't1', status: 'in_progress' }, {});
    expect(tasks).toEqual([{ id: 't1', subject: 'Write the migration', status: 'in_progress' }]);
  });
  ...
  it('REMOVES a task on the SDK deleted status (deleted means gone, not struck-through)', () => {
    ...
    const tasks = foldTaskEvent(seeded, 'taskupdate', { taskId: 't1', status: 'deleted' }, {});
    ...
    expect(foldTaskEvent(tasks, 'taskupdate', { taskId: 't9', status: 'deleted' }, {})).toEqual(tasks);
  });

  it('updates the subject when TaskUpdate carries one, else keeps the existing subject', () => {
    const seeded = [{ id: 't1', subject: 'old subject', status: 'pending' as const }];
    const renamed = foldTaskEvent(seeded, 'taskupdate', { taskId: 't1', subject: 'new subject' }, {});
    expect(renamed[0].subject).toBe('new subject');

    const untouched = foldTaskEvent(seeded, 'taskupdate', { taskId: 't1', status: 'completed' }, {});
    expect(untouched[0].subject).toBe('old subject');
  });

  it('creates a defensive entry for a TaskUpdate whose id was never created', () => {
    const tasks = foldTaskEvent([], 'taskupdate', { taskId: 't9', status: 'completed' }, {});
  ...
  it('folds dependency edges: addBlockedBy accumulates, removeBlockedBy clears, addBlocks is the inverse', () => {
    let tasks = foldTaskEvent([], 'taskcreate', { subject: 'a' }, 'Task #1 created successfully: a');
    tasks = foldTaskEvent(tasks, 'taskcreate', { subject: 'b' }, 'Task #2 created successfully: b');
    tasks = foldTaskEvent(tasks, 'taskcreate', { subject: 'c' }, 'Task #3 created successfully: c');

    // #2 waits on #1; then #1 declares it ALSO blocks #3 (the inverse edge folds onto #3.blockedBy).
    tasks = foldTaskEvent(tasks, 'taskupdate', { taskId: '2', addBlockedBy: ['1'] }, {});
    tasks = foldTaskEvent(tasks, 'taskupdate', { taskId: '1', addBlocks: ['3'] }, {});
    ...
    tasks = foldTaskEvent(tasks, 'taskupdate', { taskId: '2', status: 'in_progress' }, {});
    expect(tasks.find((t) => t.id === '2')?.blockedBy).toEqual(['1']);
    tasks = foldTaskEvent(tasks, 'taskupdate', { taskId: '2', removeBlockedBy: ['1'] }, {});
    expect(tasks.find((t) => t.id === '2')?.blockedBy).toBeUndefined();
  });

  it('preserves insertion order across multiple creates', () => {
    let tasks = foldTaskEvent([], 'taskcreate', { subject: 'first' }, { task: { id: 't1' } });
    tasks = foldTaskEvent(tasks, 'taskcreate', { subject: 'second' }, { task: { id: 't2' } });
