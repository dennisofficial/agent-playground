 ...
 *
 * The planner runs at most ONE structured call per thread: take the thread brief + the locked
 * decision record (overview + decisions) + the prior thread's handoff, return a phased plan (an
 ...
 * classifies, and a one-shot `handoff` call summarizes what a finished thread produced for the next.
 *
...
/** One step of a thread's locked plan, as the planner emits it (pre-persistence). */
...
/** The inputs to ONE thread-plan turn. */
export interface PlanThreadInput {
  ...
  decisions: Decision[];
  /** This thread's one-line brief from the upfront thread list. */
  brief: string;
  /** The prior thread's handoff note (null for the first thread). */
...
export interface PlannerLlm {
  /**
   * Produce the thread's phased plan: an ordered list of steps grounded in the decision record + the
   * prior handoff. Returns `undefined` when no LLM is available — the driver then runs the thread as a
   * single step whose brief IS the thread brief (a degraded-but-correct fallback).
   */
  ...
  /**
   * Mine the thread's plan for the NOTABLE decisions it makes (the always-ask-shaped calls), so the
   * gate can classify them against the record. Empty/`undefined` → the thread has no flagged decision.
   */
  extractDecisions(input: PlanThreadInput & { steps: PlannedStep[] }): Promise<PlannedDecision[] | undefined>;
  /**
   * Summarize what a finished thread produced (its handoff note for the next thread's plan). Returns
  ...
  /**
   * Decide how to pack a thread's ORDERED steps into execution sessions: a partition of the step
...
/**
 * The thread-planner chains. One namespace, four declarative chains — all sharing the rendered plan
...
export namespace PlannerChains {
  ...
  const HANDOFF_SYSTEM = [
    'You summarize what a just-finished thread produced so the NEXT thread can build on it. Two or three',
    'sentences: what now exists (modules/contracts/endpoints), and anything the next thread must know. Be',
  ...
  export const planThread = (llm: BaseChatModel): Runnable<PlanThreadInput, PlannedStep[]> =>
    phasesChain<PlanThreadInput>(llm, renderSystemPrompt('planner-structured-plan'), (i) =>
      renderPlanContext(i),
    ).withConfig({
      runName: 'Plan Thread',
    });
  ...
  export const reviewPlan = (
    ...
    phasesChain<PlanThreadInput & { draft: PlannedStep[] }>(
      llm,
      renderSystemPrompt('planner-structured-review'),
      (i) => {
  ...
  export const extractDecisions = (
    ...
    RunnableSequence.from<{ brief: string; steps: PlannedStep[] }, PlannedDecision[]>([
      RunnableLambda.from((i: { brief: string; steps: PlannedStep[] }) => {
        ...
        return { input: `${fence('thread_brief', i.brief)}\n\n${fence('plan', steps)}` };
      }),
      ChatPromptTemplate.fromMessages([
        new SystemMessage(renderSystemPrompt('planner-structured-extract')),
        HumanMessagePromptTemplate.fromTemplate('{input}'),
      ]),
  ...
  export const handoff = (
    ...
    ]).withConfig({ runName: 'Thread Handoff' });
    ...
    RunnableSequence.from<{ steps: PlannedStep[]; overview?: string; brief?: string }, number[][]>([
      RunnableLambda.from((i: { steps: PlannedStep[]; overview?: string; brief?: string }) => {
        ...
        const ctx = [
          ...
          i.brief ? fence('thread_brief', i.brief) : '',
        ]
          .filter(Boolean)
          .join('\n\n');
...
export function renderPlanContext(input: PlanThreadInput): string {
  ...
  return [
    ...
    fence('thread_brief', input.brief),
    ...(input.handoffIn ? [fence('prior_thread_handoff', input.handoffIn)] : []),
  ].join('\n\n');
}
