 ...
 *
 *   1. `createThread` — upsert the project/channel binding, persist the `threads` row, then
...
export class ThreadLifecycleService {
  ...
  async createThread(input: CreateThreadInput): Promise<CreatedThread> {
    ...
    // 2. Persist the threads row (origin='control', base_branch set).
  ...
  private async repoForRow(row: ThreadSandboxEntity): Promise<ProjectRepo> {
    ...
    if (!project) throw new Error(`No repos row for team=${row.org_id} project=${row.repo_id}`);
    const token = await this.creds.githubToken(row.org_id);
  ...
  /** Convert a persisted `ThreadSandboxEntity` row to an in-memory `FeatureSandbox`. */
  private rowToSandbox(row: ThreadSandboxEntity): FeatureSandbox {
