...
/** The narrow brain surface the driver needs (ship-time ADR promotion + Phase-3 halt wake) — resolved
...
export class ThreadDriver implements JobDispatcher {
  ...
  constructor(
    ...
    // Lazily resolves the brain (AgentSessionManager) for the server-initiated ADR-promotion turn,
   ...
   *  (files-as-store, mirroring the ADR store's plain `mkdir`+`writeFile`). A LIVE worktree artifact
  ...
  /**
   * FINALIZE THE BUILD. After all threads: promote durable decisions into `.atlas/adr/`, then run
  ...
  private async finalizeBuild(
    ...
    // Promote durable decisions into `.atlas/adr/` BEFORE shipping, so they ride the build commit.
    // `promoteAdr` HONORS the claim: if the ADR promotion is already claimed/complete (a concurrent finalize or
    ...
    const promoted = await this.promoteAdr(job);
    ...
    const outcome = await this.ship.ship({
      ...
      repo,
      sandbox,
      // Sweep the freshly-written ADR files into one commit (a no-op when nothing was promoted).
      commitMessage: ADR_COMMIT_MESSAGE,
      notify: (m) => this.post(route, m),
    });
    // Finalize the ADR spine off what THIS call actually did, never leaving the row stuck `running`:
    //  • promoted here AND ship at least ran the sandbox ship turn (`opened`) ⇒ COMPLETE. The ADR commit
    //    happens unconditionally inside `ship()` BEFORE it even attempts the PR (see `BuildShipService.ship`),
    //    and the branch push is part of that same sandbox turn — so the ADR files are on the remote branch
    ...
    //    the promote turn on an empty ADR delta once GitHub catches up, which is the endless-loop failure
    ...
    //    (`reconcileAdrPromotion`) re-promotes the now-`done`, pr_url-set row onto its open PR instead.
    if (promoted && outcome.opened) {
      await this.store.markAdrPromoted(job.id);
    } else if (promoted) {
      await this.store
        .setAdrPromotionStatus(job.id, 'failed')
        .catch(() => undefined);
    }
  }

  /**
   * Run the SERVER-INITIATED ADR promotion turn (full path). Claims the spine (`running`), then asks
   * the brain — lazily, to avoid the brain⇄driver module cycle — to distill THIS thread's durable
   * decisions into the worktree's `.atlas/adr/`. Best-effort: a failure marks the spine `failed`
   * (boot backstop retries) and returns false so the PR ships regardless.
   */
  private async promoteAdr(job: Job): Promise<boolean> {
    ...
    // the promote turn here is exactly the loop that made shipped jobs re-run promote_adr endlessly.
    const won = await this.store.claimAdrPromotion(job.id);
    if (!won) {
      this.logger.log(
        `job=${job.id}: ADR promotion already claimed/complete — skipping the promote turn`,
      );
      return false;
    }
    try {
      const brain = await this.brain();
      await brain.promoteAdrAtShip(job.id, job.orgId, job.repoId);
      return true;
    } catch (err) {
      this.logger.warn(
        `ADR promotion turn failed for ${job.id} (shipping anyway): ${err}`,
      );
      await this.store
        .setAdrPromotionStatus(job.id, 'failed')
        .catch(() => undefined);
