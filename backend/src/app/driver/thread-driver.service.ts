...
export class ThreadDriver implements JobDispatcher {
  ...
  async redriveThread(
    ...
    // Clear the JOB-level phase-preserving halt too: the re-drive must lift the halt or `runJob`/`drive`'s
    // halt gate would refuse to re-drive.
   ...
   * {@link relayFailure} for why a bare `post()` is invisible to reconnecting clients). A thread that ended
  ...
  private buildTurnBridge(
    ...
    const tools: Record<string, ToolImpl> = {
      ...
      complete_thread: async (args) => {
        ...
        // Task double-check: on the FIRST `done` claim, if the durable checklist still has open items, bounce
        // once (not latched) so the model reconciles its own tasks in-turn (finishing genuinely-unfinished
        // work). One reminder only; the retry skips this and proceeds, and the done transition flips any
        // leftovers to `dropped`.
        if (!taskNudgedOnce) {
          const open = (
            await this.store
        ...
        // human ship-review are the real backstops.
        terminated = 'done';
