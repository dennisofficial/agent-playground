...
export class DriverStoreService {
   ...
   *  phases are NOT re-driven here. Leave future `session_resume_at` parks alone, and do not wake dormant
   *  threads whose last turn already ended with a display halt reason — those are resumed only by an operator
   *  message/action or an explicit subscribed host event. */
  async runningJobs(): Promise<Job[]> {
    const rows = await this.jobs
      .createQueryBuilder('job')
      .where('job.status IN (:...statuses)', {
        statuses: ['building', 'master_review', 'shipping'],
      })
      .andWhere('(job.session_resume_at IS NULL OR job.session_resume_at <= now())')
      .andWhere(
        `NOT EXISTS (
          SELECT 1
          FROM "threads" t
          WHERE t."job_id" = job."id"
            ...
            AND t."status" <> 'done'
            AND t."role" IN ('builder', 'master_review')
            AND t."halt_reason" IS NOT NULL
        )`,
      )
      .getMany();
  ...
  async parkForShipReview(
    ...
    const parked = await this.dataSource.transaction(async (m) => {
      const res = await m
        ...
        .set({ status: 'ready' })
        .where('id = :jobId', { jobId })
        .andWhere("status IN ('building', 'master_review', 'amending')")
        .execute();
  ...
  async markPreviewRequested(jobId: string): Promise<boolean> {
    ...
    const res = await this.messages
      ...
      .andWhere("EXISTS (SELECT 1 FROM jobs j WHERE j.id = :jobId AND j.status = 'ready')")
    ...
    return this.dataSource.transaction(async (m) => {
      ...
      await messages.save(
        messages.create({
          ...
          card: webReplanProposalCard({ jobId, reason }) as unknown as Record<string, unknown>,
        }),
      );
      return 'posted';
    });
   ...
   */
  async neutralizeReplanProposal(jobId: string, verdictLine: string): Promise<void> {
    const rows = await this.messages.find({
      where: { job_id: jobId, ts: `replan-proposal:${jobId}`, kind: 'card' },
    });
    for (const row of rows) {
  ...
  async setThreadHaltReason(threadId: string, reason: string | null): Promise<void> {
  ...
  async setThreadCondition(_threadId: string, _condition: ThreadCondition): Promise<void> {}
  ...
  async appendThreadReviewFindings(threadId: string, findings: ReviewFinding[]): Promise<number> {
    if (findings.length === 0) {
      const row = await this.threads.findOne({
        where: { id: threadId },
        select: { review_findings: true },
      });
      return Array.isArray(row?.review_findings) ? row!.review_findings.length : 0;
    }
    const row = await this.threads.findOne({
      where: { id: threadId },
      select: { review_findings: true },
  ...
  async getPipelineState(jobId: string, orgId: string): Promise<unknown> {
    ...
    if (job.status === 'scoping') {
      ...
      return {
        ...
        mainDefaultFooter: laneDefaultFooter('planner'),
