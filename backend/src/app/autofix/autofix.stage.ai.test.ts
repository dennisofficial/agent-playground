...
describeLive('AutoFixStage framework lens — LIVE model-backed review turn', () => {
  ...
  it(
    'the model flags the injected array-index-key rule as a framework finding',
    async () => {
      ...
      writeFileSync(join(artifactDir, 'parsed-findings.json'), JSON.stringify(findings, null, 2), 'utf8');
      ...
      const haystack = findings.map((f) => `${f.title} ${f.detail}`).join(' ').toLowerCase();
