 ...
 */
const OAUTH_TOKEN =
  process.env.CLAUDE_OAUTH_TOKEN ?? process.env.CLAUDE_CODE_OAUTH_TOKEN;
...
const harnessStub = {
  create: () => {
    throw new Error(
      'harness must not be created in this test (ctx has no jobId/channel)',
    );
  },
} as unknown as TurnHarnessFactory;

describeLive(
  'AutoFixStage framework lens — LIVE model-backed review turn',
  () => {
    ...
    beforeAll(async () => {
      ...
      writeFileSync(
        join(worktree, VIOLATING_FILE_PATH),
        VIOLATING_FILE,
        'utf8',
      );
      git('add', '-A');
    ...
    it('the model flags the injected array-index-key rule as a framework finding', async () => {
      ...
      const ctx: AutoFixContext = {
        ...
        intent:
          'Add a UserList component that renders the given users as a list.',
        label: 'frontend',
        frameworkBodies: [
          { name: INJECTED_SKILL_NAME, body: INJECTED_SKILL_BODY },
        ],
      };
