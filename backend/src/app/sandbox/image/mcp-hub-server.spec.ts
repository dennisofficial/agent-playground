...
import { buildStdioSpawn, decodeRoute, diffServers, findSetpriv, validServer } from './mcp-hub-server';

const SPAWN = { uid: 501, gid: 20, cwd: '/workspace', home: '/home/atlas', baseEnv: { PATH: '/usr/bin' } };

describe('buildStdioSpawn', () => {
  const spec: ResolvedMcpServer = { name: 'fs', transport: 'stdio', command: 'npx', args: ['-y', '@fs/mcp'], env: { TOKEN: 't' } };
  ...
  it('wraps in setpriv with the host uid/gid when setpriv is available', () => {
    ...
    expect(out.args).toEqual(['--reuid', '501', '--regid', '20', '--clear-groups', '--', 'npx', '-y', '@fs/mcp']);
    expect(out.cwd).toBe('/workspace');
    // base env + HOME + the server's own env (server wins).
    expect(out.env).toEqual({ PATH: '/usr/bin', HOME: '/home/atlas', TOKEN: 't' });
  });
  ...
  it('degrades to a direct spawn when no uid/gid was supplied (even with setpriv)', () => {
    const out = buildStdioSpawn(spec, { cwd: '/workspace', home: '/home/atlas', baseEnv: {} }, '/usr/bin/setpriv');
    expect(out.command).toBe('npx');
  });
});
...
describe('diffServers', () => {
  const a: ResolvedMcpServer = { name: 'a', transport: 'http', url: 'https://a' };
  ...
  it('removes a server dropped from the desired set', () => {
    const current = new Map([['a', { key: serverKey(a) }], ['b', { key: serverKey(b) }]]);
  ...
  it('treats a changed endpoint as remove+add (reconnect) for just that server', () => {
    const current = new Map([['a', { key: serverKey(a) }]]);
    const moved: ResolvedMcpServer = { name: 'a', transport: 'http', url: 'https://a2' };
...
describe('parseHubConfig', () => {
  it('parses a well-formed config', () => {
    const cfg = { spawn: { cwd: '/workspace', home: '/home/atlas', baseEnv: {} }, servers: [] };
