...
/**
 * End-to-end proof of the submodule ship-step fix at the layer the bug actually manifests: a real Docker
 * sandbox container attached to a FULL-CLONE-provisioned submodule repo can run `git add -A` inside
 * `/workspace` and it SUCCEEDS — the exact operation that fataled in prod with `cannot chdir` /
 * `not a git repository`.
 *
 * A submodule repo is provisioned as a full local clone (`LocalGitService.createBaseClone`) — a real `.git`
 * DIR under `/workspace`, submodule gitdirs under `/workspace/.git/modules/…` — precisely because a linked
 * worktree's shared-common-dir submodule gitdirs get an unresolvable relative `core.worktree` across the
 * container's split mounts. `JobLifecycleService` makes that choice with `hasSubmodules`; this test drives
 * the same path directly. Because `.git` is a real dir inside the worktree, the attach guard
 * (`sandbox-manager.service.ts`, the `!gitDir.startsWith(worktreePath + '/')` branch) SKIPS the
 * linked-worktree overlay entirely: NO `/.atlas/git-common` bind and NO `/workspace/.git` shadow. Everything
 * lives on the single `/workspace` mount, so the submodule's relative `core.worktree` resolves in-container.
 * This test is the executable proof of that "no attach change needed" claim.
 *
...
describe('SandboxManager — submodule repo full-clone (integration, needs Docker)', () => {
  ...
  beforeAll(async () => {
    ...
    // 3. The persistent MAIN CLONE — `createBaseClone` LOCAL-clones FROM this, and `hasSubmodules` reads
    // its `origin/main:.gitmodules`.
    ...
    // 4. Provision the worktree exactly as the live path does for a submodule repo: `hasSubmodules` → full
    // clone (`createBaseClone`, NOT the linked `createBaseWorktree`) → feature branch → submodules.
