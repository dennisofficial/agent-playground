...
describe('SandboxManager.attach — onMilestone', () => {
  ...
  it('caps the sandbox: init + default memory/PID ceilings, but NO hard CPU cap, on a cold create', async () => {
    const { engine, createContainer } = fullFakeEngine(null);
    const mgr = new SandboxManager(
      engine,
      fakeBuilder(false),
  ...
  it('reinstates a hard CPU cap and passes the agent-nice override when configured', async () => {
    const { engine, createContainer } = fullFakeEngine(null);
    const mgr = new SandboxManager(
      engine,
      fakeBuilder(false),
    ...
    const spec = (
      createContainer.mock.calls[0] as unknown as [
        { nanoCpus?: number; env?: Record<string, string> },
      ]
    )[0];
    expect(spec.nanoCpus).toBe(4 * 1e9);
    expect(spec.env?.ATLAS_AGENT_NICE).toBe('15');
  });
