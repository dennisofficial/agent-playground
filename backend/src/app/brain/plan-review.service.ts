...
import { BLOCK_SINK, type BlockSink, TurnHarnessFactory, laneFor } from '../surface';
...
export class PlanReviewService {
   ...
   */
  private async ensureCodexReviewThread(jobId: string, orgId: string): Promise<string> {
  ...
  private async persistRow(
    ...
    const threadId = existing?.id ?? (await this.ensureCodexReviewThread(input.jobId, input.orgId));
