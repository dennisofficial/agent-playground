...
import { StimulusStoreService, renderTurn, type TurnChunk } from '../stimulus';
import {
  CHAT_SURFACE,
  type ChatSurface,
  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  private async deliverPendingViaFreshTurn(
    ...
    const leased = await this.stimulusStore
      ...
      .catch((err) => {
        this.logger.warn(
          `pump: leaseChatStimuli failed for thread=${jobId}: ${err}`,
        );
        return false;
      });
    if (!leased) return;
  ...
  private async steerPending(
    ...
    for (const p of pending) {
      await this.engineRunner.steer!(turnId, p.id, this.engineBody(p)).catch(
        (err) =>
          this.logger.warn(
  ...
  /**
   * Arm the precise 10s host-retry re-drive for the Main lane (d1-B), mirroring the build lane's
   * `scheduleBuildHostRetry`. Writes the durable `kind:'retry'` resume clock FIRST (awaited — the
   * restart-only backstop: if the process dies before the in-process timer fires, the leader resume sweep
   * still re-drives once this clock is due), then arms an in-process `setTimeout` that at 10s clears the
