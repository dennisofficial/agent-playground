...
const TURN = [
  line({ type: 'user', uuid: 'p1', sessionId: 's', message: { role: 'user', content: 'go' } }),
  line({ type: 'assistant', uuid: 'b-text', sessionId: 's', timestamp: '2026-06-30T12:00:01.000Z', message: { content: [{ type: 'text', text: 'hi there' }] } }),
  line({ type: 'assistant', uuid: 'b-read', sessionId: 's', timestamp: '2026-06-30T12:00:02.000Z', message: { content: [{ type: 'tool_use', id: 'tr', name: 'Read', input: {} }] } }),
  line({ type: 'user', uuid: 'b-res', sessionId: 's', message: { role: 'user', content: [{ type: 'tool_result', tool_use_id: 'tr', content: 'ok', is_error: false }] } }),
  line({ type: 'assistant', uuid: 'b-ask', sessionId: 's', timestamp: '2026-06-30T12:00:03.000Z', message: { content: [{ type: 'tool_use', id: 'ta', name: 'ask_question', input: {} }] } }),
].join('\n');
...
function makeMessages(seed: Row[] = []) {
  ...
  const repo = {
    ...
    createQueryBuilder: () => {
      ...
      qb.getRawMany = async () => {
        if (alias === 'id')
          return saved.filter((r) => r.kind === 'tool' && (r.meta as { id?: string })?.id).map((r) => ({ id: (r.meta as { id: string }).id }));
        return saved.filter((r) => (r.meta as { sdkUuid?: string })?.sdkUuid).map((r) => ({ u: (r.meta as { sdkUuid: string }).sdkUuid }));
      };
      return qb;
    },
...
describe('backfillThreadFromTurns', () => {
  ...
  it('dedupes an individual block already present by sdkUuid (turn not skipped wholesale)', async () => {
    ...
    const jsonl = [
      line({ type: 'user', uuid: 'p1', sessionId: 's', message: { role: 'user', content: 'go' } }),
      line({ type: 'assistant', uuid: 'b-read', sessionId: 's', timestamp: '2026-06-30T12:00:01.000Z', message: { content: [{ type: 'tool_use', id: 'tr', name: 'Read', input: {} }] } }),
      line({ type: 'user', uuid: 'b-res', sessionId: 's', message: { role: 'user', content: [{ type: 'tool_result', tool_use_id: 'tr', content: 'ok' }] } }),
