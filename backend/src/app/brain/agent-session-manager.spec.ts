...
describe('AgentSessionManager — direct-build turn-end latch (decision d3)', () => {
  ...
  it('flag set + running + owning feature_branch → latches the PR on the LIVE branch and promotes the ledger once', async () => {
    ...
    // The ledger promotion is kicked exactly once, on the reloaded (now done) job.
  ...
  it('does NOT AWAIT the ledger promotion (fire-and-forget — no per-job turn-queue deadlock)', async () => {
    const { manager } = makeManager();
    // A promotion turn that never resolves would hang the caller if it were awaited.
    const promote = spyPromote(manager).mockReturnValue(new Promise<void>(() => undefined));
    pending(manager).set(JOB_ID, true);

    // Resolves despite the never-resolving promotion → the latch voided it rather than awaiting.
    await expect(runLatch(manager, stimulus)).resolves.toBeUndefined();
    expect(promote).toHaveBeenCalledOnce();
  });
