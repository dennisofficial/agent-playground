...
import type { RepoAdrEntity, RepoEntity } from '../persistence/entities';
import { adrContentHash } from './adr.service';
import { RepoAdrManifestService } from './repo-adr-manifest.service';

/** A minimal merged ADR file (frontmatter the manifest parser reads + a body). */
function adrFile(over: { slug?: string; supersededBy?: string | null } = {}): string {
...
function make(opts: { existing?: Partial<RepoAdrEntity> | null; file?: string }) {
  ...
  } as unknown as Repository<RepoAdrEntity>;
  const repos = {
    findOne: vi.fn(async () => ({
      id: 'repo-1',
      org_id: 'org-1',
  ...
  const git = {
    ...
    listFilesAtRef: vi.fn(async () => ['.atlas/adr/money-out-requires-superadmin.md']),
    readFileAtRef: vi.fn(async () => opts.file ?? adrFile()),
  } as unknown as LocalGitService;
  const creds = { githubToken: vi.fn(async () => undefined) } as unknown as CredentialResolver;
  const svc = new RepoAdrManifestService(manifest, repos, git, creds);
  return { svc, manifest, updates, saved };
}

describe('RepoAdrManifestService.reconcileFromBaseCheckout', () => {
  ...
  it('baseline matches the merged file → flips proposed→accepted (no flag)', async () => {
    ...
    const { svc, updates } = make({
      existing: { id: 'm1', content_hash: adrContentHash(file), flagged: false },
      file,
    });
    const res = await svc.reconcileFromBaseCheckout('org-1', 'repo-1');
  ...
  it('frontmatter superseded_by → records status superseded', async () => {
    const file = adrFile({ supersededBy: 'newer-call' });
...
describe('RepoAdrManifestService.recordPromoted', () => {
