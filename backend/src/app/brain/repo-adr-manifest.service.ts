...
import { RepoAdrEntity, RepoEntity } from '../persistence/entities';
import { adrContentHash, parseAdrFrontmatter } from './adr.service';

/**
 * THE ADR MANIFEST MANAGER (Phase 2) — owns `repo_adrs`, the graph + freshness source of truth over the
 * committed `.atlas/adr/` files. Two write paths:
 *
 ...
 *    MERGED default-branch ADRs and (a) flip a matching proposed row to `accepted`, (b) refresh the
...
export type PromotedManifestInput = {
  slug: string;
  title: string;
  contentHash: string;
  tags: string[];
...
const ADR_DIR = '.atlas/adr';

@Injectable()
export class RepoAdrManifestService {
  private readonly logger = new Logger(RepoAdrManifestService.name);

  constructor(
    @InjectRepository(RepoAdrEntity, DB_CONNECTION)
    private readonly manifest: Repository<RepoAdrEntity>,
  ...
  /**
   * Reconcile a repo's manifest against its MERGED default-branch ADRs. Returns counts. Resolves the repo
  ...
  async reconcileFromBaseCheckout(
    ...
    const files = (await this.git.listFilesAtRef(project.repoPath, ref, ADR_DIR)).filter(
    ...
    for (const path of files) {
      ...
      const hash = adrContentHash(content);
      ...
      if (!existing.flagged) {
        this.logger.warn(
          `ADR drift: ${repoId}/${slug} on ${ref} diverged from its recorded baseline — flagging for operator review (not auto-trusted)`,
        );
      }
    ...
    if (accepted || flagged) {
      this.logger.log(
        `ADR reconcile ${repoId}: ${files.length} file(s), ${accepted} accepted, ${flagged} flagged`,
      );
    }
