...
export class BrainStoreService {
  ...
  // ── ADR promotion spine (boot backstop + direct-path stamp) ──────────────────────────

  /** Mark a thread's ADR promotion COMPLETE — stamped only after the promotion turn + commit succeed. */
  ...
  /**
   * Boot backstop: SHIPPED threads (PR opened) whose ADR promotion never reached `complete` — the
   * crash window after ship but before the ADR commit/stamp, plus a direct build whose brain skipped
   ...
   */
  async threadsAwaitingAdrPromotion(): Promise<Job[]> {
    ...
    const rows = await this.jobs.find({
      where: {
        pr_url: Not(IsNull()),
        status: Not('running'),
        adr_promotion_status: Not('complete'),
      },
    });
    // `Not('complete')` excludes NULLs in SQL, so add the never-started rows explicitly.
    const nullRows = await this.jobs.find({
      where: { pr_url: Not(IsNull()), status: Not('running'), adr_promotion_status: IsNull() },
    });
    // Onboarding threads never get `promote_adr` (see `buildTools`) — there is nothing durable
    ...
    // resurrect the impossible `promote_adr` harness turn against one.
