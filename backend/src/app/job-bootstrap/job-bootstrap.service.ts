...
export class JobBootstrapService {
  ...
  /**
   * The job's `ci` stage-thread id, or `null` before it exists (pre-ship — `DriverStoreService.ensureCiThread`
   ...
   * a later `ci` stage, and inbound events should always reach the CURRENT one.
   */
