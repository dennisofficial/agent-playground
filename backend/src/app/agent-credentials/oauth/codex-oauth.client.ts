...
/** 400/401/403 from the token endpoint mean the grant/refresh is dead — reauth, don't retry. */
...
export async function startDeviceAuth(): Promise<CodexDeviceAuth> {
  ...
  const intervalSec = Number.isFinite(intervalRaw as number) ? Number(intervalRaw) : 5;
  return {
    deviceAuthId,
    userCode,
    intervalSec,
...
/** Step 2 — poll once. HTTP 403/404 = pending; 200 = an authorization code + server PKCE verifier. */
export async function pollDeviceOnce(input: {
  deviceAuthId: string;
  userCode: string;
...
/** Step 3 — exchange the device authorization code (form-urlencoded) for tokens. */
...
function parseTokens(raw: unknown): CodexTokens {
  const body = (raw ?? {}) as {
    id_token?: unknown;
    access_token?: unknown;
    refresh_token?: unknown;
 ...
 */
export function buildAuthJson(tokens: CodexTokens, nowIso: string): string {
