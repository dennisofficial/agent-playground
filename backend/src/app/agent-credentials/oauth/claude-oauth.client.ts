...
/** 400/401/403 from the token endpoint mean the refresh/grant is dead — reauth, don't retry. */
...
/** The decrypted material shape for a Claude `personal` credential (Claude Code's `.credentials.json`). */
...
export async function refresh(refreshToken: string): Promise<ClaudeTokenSet> {
  return postForTokenSet({
...
/** Build the `.credentials.json`-shaped blob we persist (encrypted) for a personal credential. */
export function tokenSetToBlob(t: ClaudeTokenSet): ClaudeCredentialBlob {
  return {
    claudeAiOauth: {
