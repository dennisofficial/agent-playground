...
import { OrgWorktreeMountEntity, OrgWorktreeSeedEntity } from '../persistence/entities';
import type { MountMode, MountSpec } from '../sandbox/container-paths';
import { loadLegacyManifestFile } from './legacy-worktree-manifest';

/**
 * The org+repo-scoped store for a repo's worktree config — cache/auth `mounts` + golden `seed` — DB-backed
 * so a `write_worktree_config` call from any thread reaches every OTHER in-flight job's very next
 * hydration instantly, no PR/merge/rebase lag (see docs/adr/0003). Mirrors {@link WorktreeSecretStore}'s
...
export class WorktreeConfigStore {
  ...
  constructor(
    ...
    @InjectRepository(OrgWorktreeSeedEntity, DB_CONNECTION)
    private readonly seeds: Repository<OrgWorktreeSeedEntity>,
  ) {}
  ...
  /** All golden-seed paths for a repo. */
  async listSeed(orgId: string, repoId: string): Promise<string[]> {
    const rows = await this.seeds.find({ where: { org_id: orgId, repo_id: repoId } });
    return rows.map((r) => r.path);
  }

  /** Add a golden-seed path — idempotent no-op if already present. */
  async addSeed(orgId: string, repoId: string, path: string): Promise<void> {
    const existing = await this.seeds.findOne({ where: { org_id: orgId, repo_id: repoId, path } });
    if (!existing) {
      await this.seeds.save(this.seeds.create({ org_id: orgId, repo_id: repoId, path }));
    }
    this.logger.log(`added worktree seed org=${orgId} repo=${repoId} path=${path}`);
  }

  /**
   * One-time migration off a committed `atlas.json`: if this repo has ZERO mount/seed rows in the DB and
   * its worktree still has a legacy manifest file, import every entry from it, after which the DB is
  ...
  async importLegacyIfEmpty(orgId: string, repoId: string, worktreePath: string): Promise<void> {
    const [existingMounts, existingSeed] = await Promise.all([
      this.mounts.find({ where: { org_id: orgId, repo_id: repoId } }),
      this.seeds.find({ where: { org_id: orgId, repo_id: repoId } }),
    ]);
    if (existingMounts.length > 0 || existingSeed.length > 0) return;

    const { manifest } = loadLegacyManifestFile(worktreePath);
    if (manifest.mounts.length === 0 && manifest.seed.length === 0) return;

    for (const m of manifest.mounts) await this.upsertMount(orgId, repoId, m.path, m.mode);
    for (const s of manifest.seed) await this.addSeed(orgId, repoId, s);
    this.logger.log(
      `imported legacy worktree manifest org=${orgId} repo=${repoId} mounts=${manifest.mounts.length} seed=${manifest.seed.length}`,
    );
  }
}
