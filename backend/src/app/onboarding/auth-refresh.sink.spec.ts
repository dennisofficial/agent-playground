...
function fakeClaudeStore() {
  ...
  } as unknown as ClaudeCredentialStore & { advanceClaudeCredential: ReturnType<typeof vi.fn> };
}
...
describe('AuthRefreshSinkService', () => {
  ...
  it('swallows a store error so it never fails the turn-completion path', async () => {
    ...
    await expect(
      new AuthRefreshSinkService(tenantStore, claudeStore).persist({ orgId: 'org1', engine: 'codex' }, 'blob'),
