...
export class WorkspaceConfigStore {
  ...
  async removeMount(
    ...
    this.logger.log(
      `removed worktree mount org=${orgId} repo=${repoId} path=${path}`,
    );
  }
  ...
  async importLegacyIfEmpty(
    ...
    for (const m of manifest.mounts)
      await this.upsertMount(orgId, repoId, m.path, m.mode);
    this.logger.log(
