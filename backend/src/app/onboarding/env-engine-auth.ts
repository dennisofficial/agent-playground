 ...
 *  - `ENGINE_AUTH_MODE=subscription` + Claude + `CLAUDE_OAUTH_TOKEN` set → subscription;
...
export function engineAuthFromEnv(
  ...
  const mode = env.get('ENGINE_AUTH_MODE') ?? 'api_key';
  if (mode === 'subscription') {
    if (engine === 'claude') {
      const secret = env.get('CLAUDE_OAUTH_TOKEN');
