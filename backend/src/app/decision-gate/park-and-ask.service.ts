...
/**
 * Where to park-and-ask: a thread reference Atlas talks back into. The thread is parked ON this
...
/** A live park — the question is posted, the thread is suspended awaiting a human reply. */
export interface ParkHandle {
   ...
   * this to block the thread, OR ignore it and poll `resolved`/`resolution` (non-blocking).
   */
...
/**
 * W5 — the PARK-AND-ASK mechanism. When the classifier returns `ask`, the thread driver hands the
 ...
 *
 * Non-blocking by contract: the driver may `await handle.answer` (suspend the thread) OR poll
 * `handle.resolved` / `handle.resolution` and keep other threads moving — the thread is "parked",
 * not the process.
 *
 ...
 * deferred behind this clear seam: persist `ParkState` to an `app` row on `register`, rehydrate +
...
export class ParkAndAskService implements OnModuleDestroy {
  ...
  /**
   * Park a thread and ask the human a question in the target thread. Posts the question, registers the
  ...
  /** Abandon a park (e.g. the thread was cancelled). Rejects its `answer` and drops it. */
