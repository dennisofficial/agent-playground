...
/**
 * Where to park-and-ask: a thread reference Atlas talks back into. The track is parked ON this
...
/** A live park — the question is posted, the track is suspended awaiting a human reply. */
export interface ParkHandle {
   ...
   * this to block the track, OR ignore it and poll `resolved`/`resolution` (non-blocking).
   */
...
/**
 * W5 — the PARK-AND-ASK mechanism. When the classifier returns `ask`, the track driver hands the
 ...
 *
 * Non-blocking by contract: the driver may `await handle.answer` (suspend the track) OR poll
 * `handle.resolved` / `handle.resolution` and keep other tracks moving — the track is "parked",
 * not the process.
 *
...
export class ParkAndAskService implements OnModuleDestroy {
  ...
  /**
   * Park a track and ask the human a question in the target thread. Posts the question, registers the
  ...
  /** Abandon a park (e.g. the track was cancelled). Rejects its `answer` and drops it. */
