...
/** A track's plan to surface for human visibility. */
export interface SectionPlanPost {
  ...
  orgId?: string;
  /** A short track title ("Backend — persistence layer"). */
  title: string;
  /** The detailed plan prose / step list. */
  plan: string;
  /**
   * The never-ask / covered decisions made WHILE planning this track — surfaced here so the human can
...
/**
 * W5 — VISIBILITY posting. Posts a track's detailed plan into the job thread for human visibility.
 * NON-BLOCKING and never gates: the track proceeds whether or not anyone reads it; the human may
...
export class PlanVisibilityService {
  ...
  /**
   * Post the track plan into its thread. Returns the posted message's ts (or undefined if no surface
  ...
  async postSectionPlan(post: SectionPlanPost): Promise<string | undefined> {
    ...
    try {
      ...
      this.logger.warn(
        `failed to post track plan "${post.title}": ${err instanceof Error ? err.message : String(err)}`,
      );
      return undefined;
    }
  }
...
/** Render the track plan as a plain-text Slack post (mrkdwn). Pure — unit-testable. */
