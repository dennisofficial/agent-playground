...
describe('codexAuthAdapter.materialize', () => {
  it('writes auth.json verbatim for a valid blob (round-trip)', () => {
    ...
    const home = codexAuthAdapter.materialize({ homeRoot: root, key, secret, env });
  ...
  it('throws on invalid JSON', () => {
    const root = mkdtempSync(join(tmpdir(), 'codex-adapter-'));
    expect(() => codexAuthAdapter.materialize({ homeRoot: root, key, secret: 'not-json', env: {} })).toThrow();
  });
});
...
describe('codexAuthAdapter.readBackRefresh', () => {
  it('detects a change (refresh) and returns the rotated blob', () => {
    ...
    codexAuthAdapter.materialize({ homeRoot: root, key, secret: written, env: {} });

    const rotated = JSON.stringify({ ...fullTokens, last_refresh: '2026-07-04T00:00:00.000Z' });
    codexAuthAdapter.materialize({ homeRoot: root, key, secret: rotated, env: {} });

    expect(codexAuthAdapter.readBackRefresh({ homeRoot: root, key, writtenSecret: written })).toBe(rotated);
  });
  ...
  it('returns undefined when unchanged', () => {
    ...
    expect(codexAuthAdapter.readBackRefresh({ homeRoot: root, key, writtenSecret: written })).toBeUndefined();
  });

  it('returns undefined when the file is absent', () => {
    const root = mkdtempSync(join(tmpdir(), 'codex-adapter-'));
    expect(
      codexAuthAdapter.readBackRefresh({ homeRoot: root, key: { ...key, jobId: 'never-written' }, writtenSecret: 'x' }),
    ).toBeUndefined();
  });
});
