...
export class ToolBridgeHost {
  ...
  private async _dispatch(req: ToolRequestFrame): Promise<void> {
    ...
    // Per-thread scoping: reject requests that carry a different thread id in their args.
    // Convention: if args includes a `threadId` field it must match the owning thread.
    if (typeof args['threadId'] === 'string' && args['threadId'] !== this.bridge.threadId) {
      this._respond({ t: 'tool_error', id, message: `Thread scope violation: tool '${name}' requested for thread '${args['threadId']}' but this exec belongs to thread '${this.bridge.threadId}'` });
      return;
    }

    const impl = this.bridge.tools[name];
    if (!impl) {
      this._respond({ t: 'tool_error', id, message: `Unknown tool: '${name}'` });
      return;
    }

    try {
      const result = await impl(args);
      this._respond({ t: 'tool_response', id, result });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      this._respond({ t: 'tool_error', id, message });
    }
  }
