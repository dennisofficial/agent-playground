...
**Two more live thread groups take over what Main used to do at the tail end.** `post_build` (spawned once all
build/direct*build thread groups + `master_review` complete — `DriverStoreService.ensurePostBuildThread`, called
...
message (`post_build` gets the same "ship now" user message Main used to receive). Dedicated per-thread-group
