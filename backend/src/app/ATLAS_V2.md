...
- **The single central brain + `Stimulus` router** — `TriageService`, `ConversationalBrainService`,
  ...
  its own continuous Claude Code session (the _job brain_), and events are delivered to it as harness
...
⊃ `Repos` ⊃ `Threads` (channels removed; a thread is the conversation container _and_ the build unit).
...
own continuous Claude Code session (the _thread brain_); see `ARCHITECTURE.md` §4 + §7. The `Stimulus`
...
Env loads via `pnpm dev:env` = `dotenvx run -f .env.local.enc -f .env.personal`. **Key env vars** (all optional, in `_core/config/env/validation.ts`): `SURFACE`(web|agent, default web), `HTTP_PORT`(4002), `GITHUB_TOKEN`/`GITHUB_TOKEN`, `GITHUB_WEBHOOK_SECRET`, `WEBHOOK_SECRET`, `REPOS_ROOT`, `AGENT_HOME_ROOT`, `CLAUDE_OAUTH_TOKEN`/`CODEX_OAUTH_TOKEN`(SDK harness subscription secrets — the harness is subscription-only, no api*key mode), `EVENT_DEDUP_WINDOW_S`, `EVENT_RATE_LIMIT`/`_WINDOW_S`, `MAX_SECTIONS`(12), `MAX_PHASES_PER_SECTION`(8), `WORKER_MODEL`/`CODEX_MODEL`. Reuses `ANTHROPIC_API_KEY`, `CHAT_MODEL`/`GATE_MODEL`, `POSTGRES*\*`, `SLACK_BOT_TOKEN`/`SLACK_APP_TOKEN`.
...
> ⏳ **Point-in-time snapshot** (the W-stream notation is retired). For the _current_ built-vs-planned
...
**Bug found + fixed during W9:** mid-grill, a human follow-up was being **re-triaged and dropped** (continuity check looked for an open _job_, but none exists during the question phase). Fixed by anchoring a `scoping` job at triage time (`triage.service.ts`); regression-tested in `triage.service.spec.ts`.
...
1. **Builds are slow** (~7 min for a trivial README change). The planner inserts an "investigate" section first and runs the 3-lens auto-fix on _every_ section incl. zero-change ones. Many LLM turns for small work. Tuning: planner shouldn't over-decompose; skip auto-fix on 0-change sections; the e2e PR poll (now 420s in `e2e-harness.service.ts`) still missed PR #45 by ~6s.
2. **Autonomy/security triage is non-deterministic.** The real triage LLM sometimes PARKS an untrusted event (safe), sometimes summarizes it as a benign job and DISPATCHES (e2e runs 1-2 parked the injection; run 3 dispatched a benign "fix CI failure"). The injected **destructive action never executed**, but the triage-level guard isn't reliable — it relies on the driver's per-section gate as defense-in-depth. **RECOMMENDATION: move the autonomy gate out of triage** (which classifies an _unplanned_ fix it can't verify) **into the driver's per-section gate** (where real decisions are known) — both more deterministic and the right layer. The classifier itself is deterministic on rules; the non-determinism is the triage LLM's event→summary step.
...
| Base image        | `sandbox/image/Dockerfile` + `sandbox-init.sh` + `sandbox/sandbox-image.builder.ts`               | node+pnpm+git+docker+dockerd + the atlas-engine-turn launcher + the 2 SDKs; PID1 starts inner dockerd. Boot-memoized build (tag `SANDBOX_IMAGE`, default `atlas-sandbox:latest`)                                                                                                                                                                                                                                                                                                                                                                                     |
| Engine app        | `src/engine` → `nest build engine --webpack` → `sandbox/engine-app.js` + `.map`                   | runs the Nest engine app in-container over Redis Streams. **HOT — the API refreshes the built engine app on every (re)start (`ensureEngineApp`, via `SandboxImageBuilder`) and bind-mounts it live into every sandbox**, so engine code edits land on the NEXT turn — NO image rebuild, NO container recreate. (The Dockerfile bakes the launcher, while the live app bundle mount overrides the baked copy.) A dev `--watch` rebuild and a prod deploy-restart both refresh the bundle. |
...
**Open follow-ups (designed, not built):** human-facing dev-server **exposure** (reverse-proxy by hostname / TLS — the next plan; per-sandbox networks + labels are in place); the `/refs` per-repo-token registry + refresh/GC + an agent tool; TTL reaping of idle _running_ sandboxes (needs job-state awareness — only stopped ones are reaped now); pinning the in-image SDK versions to the host via build args; rebuild the image for the deploy arch (built linux/arm64 on the Mac; OVH is x64); prune the dead `WORKSPACE_*` env vars; the full `e2e --live` docker run.
...
- **Preserve:** `TurnRunnerService` persists `session_id` onto the phase row _on the auth error_ (previously only on success) — the resume handle. The agent's partial edits are already on disk in the (bind-mounted) worktree.
...
> sold/hosted commercial product. The isolation described below is _private multi-workspace separation
> between trusted friends_ (defence-in-depth on a family-only host), not a hardened boundary for
