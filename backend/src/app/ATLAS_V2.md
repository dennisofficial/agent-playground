...
```
Env loads via `pnpm dev:env` = `dotenvx run -f .env.local.enc -f .env.personal`. **Key env vars** (all optional, in `_core/config/env/validation.ts`): `SURFACE`(web|agent, default web), `HTTP_PORT`(4002), `GITHUB_TOKEN`/`GITHUB_TOKEN`, `GITHUB_WEBHOOK_SECRET`, `WEBHOOK_SECRET`, `REPOS_ROOT`, `AGENT_HOME_ROOT`, `ENGINE_AUTH_MODE`(api_key|subscription), `CLAUDE_OAUTH_TOKEN`, `EVENT_DEDUP_WINDOW_S`, `EVENT_RATE_LIMIT`/`_WINDOW_S`, `MAX_SECTIONS`(12), `MAX_PHASES_PER_SECTION`(8), `WORKER_MODEL`/`CODEX_MODEL`. Reuses `ANTHROPIC_API_KEY`, `CHAT_MODEL`/`GATE_MODEL`, `POSTGRES_*`, `SLACK_BOT_TOKEN`/`SLACK_APP_TOKEN`.
