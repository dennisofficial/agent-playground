...
/**
 * Root for all of Atlas's durable HOST-side runtime state — engine homes, repo clones/worktrees, and
 * the hydration sidecar. Deliberately kept INSIDE the repo at gitignored `.atlas-state/` so the whole
 * generated working set is self-contained: delete the checkout and every byte of state goes with it,
 * nothing left orphaned under the developer's `$HOME`. (Previously these defaulted under
 * `~/.agent-playground/…`, which survived a `rm -rf` of the repo.)
 *
 ...
 * `ATLAS_HYDRATION_STATE`) to point at a persistent volume in deployment, where the repo checkout is
 * ephemeral and state must outlive it.
 */
...
export function repoStateDir(name: 'agent-home' | 'repos' | 'hydration-state'): string {
