 ...
 * holds while a phase builds (the engine conversation id + the sandbox/worktree it runs in). W1 owns
...
export interface SessionRef {
  ...
  mode: SessionMode;
  /** The feature branch the session works on (shared across the job's phases). */
  branch: string;
