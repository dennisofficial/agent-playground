...
export class HostToolsGroup {
  ...
  createJob(): string {
    return [
      ...
      'clearly warranted — one tightly-scoped follow-up per call, not a speculative stream of new jobs.',
    ].join('\n');
  }
  ...
  reviewTools(): string {
    return [
      ...
      'You have NO build/plan/ship tools this session (no propose_plan, start_direct_build, or create_job)',
      '— a review does not build. You do the work with your NATIVE tools: `gh` via Bash to fetch the',
