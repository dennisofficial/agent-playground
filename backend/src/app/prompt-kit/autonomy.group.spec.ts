...
describe('autonomy.group — autonomous-mode announcement', () => {
  it.each(['plan', 'ship', 'both'] as const)(
    ...
    (mode) => {
      const out = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: mode } });
  ...
  it('emits NOTHING (byte-identical) for ATLAS_MAIN when autoApproveMode is off', () => {
    ...
    const out = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'off' } });
  ...
  it('never reaches a worker agent, even with autoApproveMode ON', () => {
    const out = renderAgentPrompt(Agent.WORKER, { settings: { autoApproveMode: 'both' } });
    expect(out).not.toContain(MARKER);
  });

  it('"both" mentions BOTH the plan-approval and ship-review gates advancing without a human', () => {
    const out = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'both' } });
  ...
  it('"plan" uses the plan-gate-only header and says the ship-review gate is UNCHANGED', () => {
    const out = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'plan' } });
  ...
  it('"ship" uses the ship-gate-only header and says the plan-approval gate is UNCHANGED', () => {
    const out = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'ship' } });
  ...
  it('the copy differs across plan / ship / both (distinct, not shared boilerplate)', () => {
    const plan = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'plan' } });
    const ship = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'ship' } });
    const both = renderAgentPrompt(Agent.ATLAS_MAIN, { settings: { autoApproveMode: 'both' } });
