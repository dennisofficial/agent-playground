/**
 * prompt-kit / bodies / meta — the META LLM-chain system prompts (relocated verbatim from their
 * original homes): the Codex plan-review turn, the decision-class gate's ambiguous-case classifier,
 * and the thread-title chain. These are small, focused, non-agentic (or reviewer) system prompts that
 * sit alongside — not inside — the main brain/worker/planner personas.
 */
 ...
 *
 * Relocated verbatim from `brain/plan-review.service.ts` (`REVIEW_SYSTEM`).
 */
...
/**
 * System prompt for the decision-class gate's ambiguous-case LLM classifier — a cheap, structured Haiku
 * call that adjudicates whether a proposed decision must be asked or may proceed autonomously.
 *
 * Relocated verbatim from `decision-gate/classifier-llm.ts` (`ClassifyDecisionChain.SYSTEM`).
 */
export const DECISION_CLASSIFIER_SYSTEM = [
...
/**
 * System prompt for the thread-title chain — a tiny non-agentic call that turns a thread's first
 * message into a short, human-readable title.
 *
 * Relocated verbatim from `titling/job-title.chain.ts` (`JobTitleChain.SYSTEM`).
 */
export const JOB_TITLER_SYSTEM = `
