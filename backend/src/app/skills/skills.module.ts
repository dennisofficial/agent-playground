 ...
 * thread `RunEngineArgs.skills`, plus the git installer/updater (P2). `@Global` (like `McpModule`) so those
 * factories — and `JobLifecycleService` for `SkillUpdaterService.reconcileOrgAsync` — inject this module's
 * providers with zero per-module import churn. `GitModule` isn't itself `@Global`, so it's imported here
 * for `LocalGitService` (installer/updater clone + `ls-remote`).
 */
@Global()
@Module({
