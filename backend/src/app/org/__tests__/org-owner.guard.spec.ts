...
describe('OrgOwnerGuard', () => {
  ...
  it('allows the owner', () => {
    expect(guard.canActivate(ctxWithOrg({ id: 'O1', role: 'owner' }))).toBe(true);
  });

  it('denies plain members', () => {
    expect(() => guard.canActivate(ctxWithOrg({ id: 'O1', role: 'member' }))).toThrow(
      ForbiddenException,
    );
  });

  it('denies an unknown role', () => {
    expect(() => guard.canActivate(ctxWithOrg({ id: 'O1', role: 'operator' }))).toThrow(
      ForbiddenException,
    );
  });

  it('fails closed when membership never ran (no org on request)', () => {
    expect(() => guard.canActivate(ctxWithOrg(undefined))).toThrow(ForbiddenException);
  });
});
