 ...
 *   --repo <https-url>      the GitHub repo (env: GATE_REPO)
 ...
 *
 * Run (dry-run):  pnpm gate -- --repo https://github.com/<owner>/<repo>
 * Run (live):     pnpm gate -- --live --repo https://github.com/<owner>/<repo>
 */
function parseArgs(argv: string[]): { config: GateConfig | null; error?: string } {
  ...
  const gitUrl = get('--repo') ?? process.env.GATE_REPO;
  if (!gitUrl) {
    return {
      config: null,
      error:
        'Missing repo. Pass --repo https://github.com/<owner>/<repo> (or set GATE_REPO).',
    };
  }
