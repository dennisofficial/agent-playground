  ...
  async run(config: GateConfig): Promise<GateResult> {
    ...
    try {
      ...
      const turn = await this.engine.run({
        ...
        task:
          `Create or append a single line to a file named GATE.md in the repo root: ` +
      ...
      if (config.dryRun) {
        record(
          ...
          token
            ...
            : 'DRY-RUN: no GitHub token — set GITHUB_TOKEN for the live gate',
        );
      } else {
        if (!token) {
          record('pull-request', false, 'no GitHub token (GITHUB_TOKEN/GITHUB_TOKEN)');
          return { ok: false, steps };
        }
