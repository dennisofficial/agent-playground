...
describe('gitAuthEnv (token never in argv / config)', () => {
  it('encodes the token as an http.extraheader GIT_CONFIG_* env var', () => {
    ...
    const expected = Buffer.from('x-access-token:ghp_secret').toString(
      'base64',
    );
  ...
  it('sameGitUrl tolerates trailing .git / slash', () => {
    expect(
      sameGitUrl('https://github.com/a/b.git', 'https://github.com/a/b/'),
    ).toBe(true);
  });
});
...
describe('gitCredHelperEnv (file-backed, url-scoped)', () => {
  ...
  it('returns {} for non-github urls — the helper never scopes to a non-GitHub remote', () => {
    expect(
      gitCredHelperEnv('file:///tmp/x.git', '/.atlas/github-token'),
    ).toEqual({});
    expect(
      gitCredHelperEnv('git@github.com:acme/app.git', '/.atlas/github-token'),
    ).toEqual({});
  });
});
