 ...
 * sequence of its thread groups: `planning | plan_review | build | direct_build | master_review |
 * post_build | ci`. EVERY thread belongs to exactly one thread group (`threads.thread_group_id` NOT NULL) — there
 * is no job-level ungrouped thread, so even a pure-chat job has exactly one `planning` thread group.
 *
 * A `build`/`direct_build` thread group owns MULTIPLE threads (sequential builder legs + review_agent(s) +
 ...
 * appends a fresh `planning`→…→`post_build`/`ci` run after the prior thread groups, which stay as visible
...
export class ThreadGroupEntity extends TimestampedEntity {
  ...
  /** The thread-group KIND — `planning | plan_review | build | direct_build | master_review | post_build |
   *  ci`. The single differentiator; behavior lives in the thread-group-kind registry (thread 2), not here
   *  (d8). No column default — every write site sets it explicitly. Stays `text` (no DB enum) — the app
   *  layer validates via `coerceThreadGroupKind` (thread 2). */
  @Column({ type: 'text' })
  kind!: string;

  /** Human label — NULLABLE, populated only for `build`/`direct_build` thread groups (the slice name, e.g.
   ...
   *  Nullable — only meaningful for `build`/`direct_build` thread groups. Stays `text` (app-layer
   *  validated). */
  @Column({ type: 'text', nullable: true })
  type!: string | null;
  ...
  /**
   * Kind-specific PARAMS (d8) — e.g. a `build` thread group's diff range hints, a `plan_review` thread
