...
/**
 * THE DECISION-LEDGER MANIFEST (Phase 2) — the GRAPH + FRESHNESS source of truth for a repo's durable
 * `.atlas/decisions/` ledger. The committed `.md` files hold the PROSE; this row holds the metadata the
 * management layer must own to keep the ledger consistent: the canonical (merged) status, the
...
export class RepoDecisionEntity extends TimestampedEntity {
  ...
  /** The repo whose ledger this belongs to (FK → repos.id). */
  @Column({ type: 'uuid' })
  repo_id!: string;
  ...
  /** Stable topic slug = the `.atlas/decisions/<slug>.md` filename + the file's frontmatter `id`. */
  @Column({ type: 'text' })
  slug!: string;
