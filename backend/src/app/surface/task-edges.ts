...
export function inverseEdgeOps(
  ...
  return [
    ...strArr(input.addBlocks).map(
      (targetId): InverseEdgeOp => ({ targetId, op: 'add' }),
    ),
    ...strArr(input.removeBlocks).map(
      (targetId): InverseEdgeOp => ({ targetId, op: 'remove' }),
    ),
  ];
}
