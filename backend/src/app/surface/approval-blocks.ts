...
/** The SHIP-REVIEW gate's "Ship it" button — the SECOND human gate (after `APPROVE_ACTION_ID` at the
 *  planning thread group). Clicked at `awaiting_ship_review` to open the PR; its button `value` carries only `{ jobId }`
