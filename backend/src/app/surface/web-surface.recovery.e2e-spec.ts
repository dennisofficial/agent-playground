...
function judgeUnavailableRecord(staticChecksAdequate: boolean): ThreadTerminalRecord {
  return {
    status: 'blocked',
    summary: 'Backend recovery mechanics HTTP e2e — work complete, held on judge outage.',
    blocked: {
...
beforeAll(async () => {
  ...
  threads = app.get<Repository<ThreadEntity>>(getRepositoryToken(ThreadEntity, DB_CONNECTION));
...
describe('Recovery mechanics — POST retry-verification / accept (live AppModule, real HTTP, real cookie auth)', () => {
  it('POST …/accept refuses over HTTP when static (build/test) checks have not passed (d4 safety split)', async () => {
    const res = await request(server)
      .post(`/web/orgs/${ORG}/repos/${REPO}/jobs/${JOB_ACCEPT_REFUSE}/threads/${THREAD_ACCEPT_REFUSE}/accept`)
      .set('Cookie', ownerCookie)
      .send({});

    // eslint-disable-next-line no-console -- evidence: dump the OBSERVED live HTTP response verbatim.
    console.log('OBSERVED POST .../accept [static-not-passed]:', res.status, JSON.stringify(res.body, null, 2));
    ...
    const reloaded = await threads.findOne({ where: { id: THREAD_ACCEPT_REFUSE } });
  ...
  it('POST …/retry-verification refuses over HTTP when the hold is not a judge outage', async () => {
    ...
    // eslint-disable-next-line no-console -- evidence: dump the OBSERVED live HTTP response verbatim.
    console.log('OBSERVED POST .../retry-verification [not-judge-outage]:', res.status, JSON.stringify(res.body, null, 2));
    ...
    const reloaded = await threads.findOne({ where: { id: THREAD_RETRY_REFUSE } });
  ...
  it('POST …/accept succeeds over HTTP for a genuine judge_unavailable hold with static checks adequate', async () => {
    const res = await request(server)
      .post(`/web/orgs/${ORG}/repos/${REPO}/jobs/${JOB_ACCEPT_OK}/threads/${THREAD_ACCEPT_OK}/accept`)
      .set('Cookie', ownerCookie)
      .send({});

    // eslint-disable-next-line no-console -- evidence: dump the OBSERVED live HTTP response verbatim.
    console.log('OBSERVED POST .../accept [happy path]:', res.status, JSON.stringify(res.body, null, 2));
  ...
  it('POST …/retry-verification with NO auth cookie is rejected (real AuthGuard enforced, not bypassed)', async () => {
    ...
    // eslint-disable-next-line no-console -- evidence: dump the OBSERVED live HTTP response verbatim.
    console.log('OBSERVED POST .../retry-verification [no cookie]:', res.status, JSON.stringify(res.body, null, 2));
  ...
  it('POST …/accept 404s on a job that does not belong to (or exist in) the caller org (requireThread scoping)', async () => {
    const res = await request(server)
      .post(`/web/orgs/${ORG}/repos/${REPO}/jobs/${JOB_NONEXISTENT}/threads/${THREAD_NONEXISTENT}/accept`)
      .set('Cookie', ownerCookie)
      .send({});

    // eslint-disable-next-line no-console -- evidence: dump the OBSERVED live HTTP response verbatim.
    console.log('OBSERVED POST .../accept [org-scoping 404]:', res.status, JSON.stringify(res.body, null, 2));
