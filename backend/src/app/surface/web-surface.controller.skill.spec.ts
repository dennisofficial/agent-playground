...
function makeController(card: unknown, ctxRoot: string) {
  ...
  const controller = new WebSurfaceController(
    ...
    {} as never, // dispatcher
    {} as never, // secrets
    { getSkillProposalCard: m.getSkillProposalCard, markSkillProposalApproved: m.markSkillProposalApproved } as never, // store
    {} as never, // brain
    {} as never, // mcpStore
    {} as never, // mcpProbe
...
describe('WebSurfaceController — skill proposal approve (owner-gated)', () => {
  ...
  it('create mode vendors the FROZEN staging copy (not the live draft), writes a custom all-surfaces row, ' +
     'and clears the /context draft', async () => {
  ...
  it('remove mode deletes the row + the skill dir', async () => {
    const card = { type: 'skill_proposal_card', mode: 'remove', scope: 'repo', repoId: 'repo-1', name: 'old', description: '' };
    const { controller, m } = makeController(card, ctxRoot);
