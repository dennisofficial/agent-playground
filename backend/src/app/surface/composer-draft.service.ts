...
export class ComposerDraftService {
   ...
   * keeping any that are stale/rejected), and blank the text + queued comments. Always an UPDATE, never a
   * DELETE (see `ComposerDraftEntity`'s doc comment) — a no-op when the caller has no draft row.
   */
  async clearOnSend(
    orgId: string,
    jobId: string,
