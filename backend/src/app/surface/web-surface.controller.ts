...
export class WebSurfaceController {
  ...
  async postMessage(
    ...
    const attach =
      userItem && files?.length ? await this.ingestAttachments(org.id, jobId, files) : null;
  ...
  async focusThread(
    ...
    if (!thread || thread.jobId !== jobId) throw new NotFoundException('thread not found');
  ...
  async setAutoApprove(
    ...
    } else if (job.status === 'ready' && modeApprovesShip(body.mode)) {
  ...
  async deleteThread(
    ...
    if (claimed) {
      ...
      void this.threadLifecycle
        ...
        .catch((err) =>
          this.logger.warn(`web delete: background teardown failed for job ${jobId}: ${err}`),
        );
    }
