...
describe('ThreadTitleService.generateAndApply', () => {
  ...
  it('does NOT emit when no row changed (placeholder already renamed / thread deleted)', async () => {
    const { service, emitThreadMeta, update } = makeService(fakeFactory('A Title'), { affected: 0 });
  ...
  it('no-ops (no update, no emit) when no Anthropic key resolves', async () => {
    const { service, emitThreadMeta, update } = makeService(fakeFactory(null), { affected: 1 });
