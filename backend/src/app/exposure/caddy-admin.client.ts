...
export class CaddyAdminClient {
  ...
  async upsertRoute(input: UpsertRouteInput): Promise<void> {
    ...
    if (existing.status === 200) {
      ...
      this.expectOk(
        await this.call('PATCH', `/id/${input.id}`, body),
        `replace route ${input.id}`,
      );
      return;
    }
    if (existing.status !== 404) {
      throw new Error(
        `caddy GET /id/${input.id} → ${existing.status}: ${existing.body}`,
      );
    }
  ...
  async deleteRoutesByPrefix(prefix: string): Promise<void> {
    const ids = (await this.listRouteIds()).filter((id) =>
  ...
  private async serverName(): Promise<string> {
    ...
    try {
      ...
      const servers = JSON.parse(res.body || '{}') as Record<
        string,
        { listen?: string[] }
      >;
      const names = Object.keys(servers);
      const tls = names.find((n) =>
        (servers[n]?.listen ?? []).some((l) => l.endsWith(':443')),
      );
      ...
      this.logger.debug(
        `caddy server discovery failed, using ${FALLBACK_SERVER}: ${err}`,
      );
