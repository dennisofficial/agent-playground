...
const kits: Record<'stripe' | 'braintree', () => PaymentProviderKit> = {
  ...
  braintree: () => ({ client: new BraintreeClient(), webhookVerifier: new BraintreeWebhookVerifier() }),
};
...
class QueryBuilder<T> {
  ...
  where(filters: Partial<T>): this { this.filters = { ...this.filters, ...filters }; return this; }
...
const baseDefaults: RequestDefaults = { timeoutMs: 5000, retries: 2, headers: { Accept: 'application/json' } };

function withOverrides(base: RequestDefaults, overrides: Partial<RequestDefaults>): RequestDefaults {
