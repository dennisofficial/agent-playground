...
**Use when.**
- You have two dimensions that multiply (e.g. notification *type* × delivery *channel*) and subclassing every combination is exploding.
...
const generator = new ReportGenerator(process.env.NODE_ENV === 'test' ? new MemoryStore() : new S3Store());
```

**Lighter alternative.** Constructor injection of a plain interface (above, and idiomatic in Nest anyway) — that already *is* Bridge. Don't add a separate parallel class hierarchy on top; the DI seam is the pattern.
...
**Lighter alternative.** A higher-order function that wraps another function (above) — this is Decorator with zero class ceremony. Reach for wrapper *classes* only when the thing being decorated has multiple methods that all need wrapping together (e.g. a repository interface).
...
**Lighter alternative.** This is functionally identical to Decorator when the goal is caching/logging (above); use the term "Proxy" specifically when the intent is *access control* (auth, lazy init) rather than adding behavior — otherwise just call it a wrapper and move on.
