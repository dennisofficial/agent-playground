 ...
 * reaper never fires (no `ATLAS_MAX_CONCURRENT_SANDBOXES`).
 *
 * Self-contained (just dockerode) so it can run as a one-shot on the host without booting the app.
 * Connects over `ATLAS_DOCKER_SOCKET_PATH ?? DOCKER_SOCKET_PATH` (else dockerode's default socket).
 *
 *   Preview:  pnpm atlas:reap -- --dry-run
 *   Reap:     pnpm atlas:reap
 */
import Docker from 'dockerode';

async function main(): Promise<void> {
  const dryRun = process.argv.includes('--dry-run');
  const socketPath = process.env.ATLAS_DOCKER_SOCKET_PATH ?? process.env.DOCKER_SOCKET_PATH;
