...
/**
 * Live e2e breakpoint coverage against the real running stack (Postgres :5433 / Nest :4002 /
 * Next :3000) — no mocks. Viewport widths are derived from the same {@link BREAKPOINTS} the app
...
test.describe("org settings route — responsive tiers", () => {
  for (const [tier, viewport] of Object.entries(VIEWPORTS)) {
    test(`${tier}: settings render with no horizontal overflow`, async ({
      page,
    }) => {
  ...
  test("mobile: settings nav collapses to a drawer opened via its hamburger", async ({
    page,
  }) => {
    await page.setViewportSize(VIEWPORTS.mobile);
    await page.goto(SETTINGS_PATH);
  ...
  test("tablet (md, above the settings' own mobile-only breakpoint): nav renders inline", async ({
    ...
    await expect(
      page.getByRole("button", { name: "Open settings navigation" }),
    ).not.toBeVisible();
  });
});
