...
The product model is **Organization → Repos → Threads**. A _thread_ (a unit of work) always lives on a
_repo_ (a connected GitHub repository). Today there is **no screen anywhere to connect or manage a repo**,
...
This tab fills that gap. It's the home for everything an operator does to a repo _except_ using it.
...
mis-points to Credentials). So the loop is: _create thread → no repo → "Connect a repo" → this Repos tab →
connect → back to creating a thread._ Please make the empty/connect states feel like a natural landing for
