...
function todoSummary(i: Record<string, unknown>): string {
  ...
  const done = todos.filter((t) => str(asRecord(t).status) === 'completed').length;
...
const NATIVE: Record<string, NativeSpec> = {
  ...
  bashoutput: { label: 'Bash output', icon: 'bash', color: 'var(--accent)', arg: (i) => str(i.bash_id ?? i.shell_id) },
  killshell: { label: 'Kill shell', icon: 'bash', color: 'var(--accent)', arg: (i) => str(i.shell_id ?? i.bash_id) },
  killbash: { label: 'Kill shell', icon: 'bash', color: 'var(--accent)', arg: (i) => str(i.shell_id ?? i.bash_id) },
  exitplanmode: { label: 'Exit plan mode', icon: 'plan', color: 'var(--accent)', arg: () => '' },
  slashcommand: { label: 'Command', icon: 'bash', color: 'var(--accent)', arg: (i) => str(i.command) },
};
...
function TodoBody({ tool }: { tool: ToolItem }) {
  const todos = Array.isArray(asRecord(tool.input).todos) ? (asRecord(tool.input).todos as unknown[]) : [];
  if (!todos.length) {
    return <StructuredPanel input={formatPayload(tool.input)} result={formatPayload(tool.result)} isError={tool.isError} />;
  }
  return (
    <div className="my-[3px] rounded-[7px] border border-border px-[11px] py-2" style={{ background: 'var(--panel)' }}>
      {todos.map((raw, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-2 py-[3px] text-[12px]">
            <span
              ...
              style={{ background: dot.fill ? dot.color : 'transparent', border: `1.5px solid ${dot.color}` }}
            />
            <span style={{ color: done ? 'var(--faint)' : 'var(--text)', textDecoration: done ? 'line-through' : 'none' }}>{content}</span>
          </div>
        );
      })}
    </div>
...
export const nativeMiscHandler: ToolHandler = {
  id: 'native-misc',
  match: (name) => Object.prototype.hasOwnProperty.call(NATIVE, name.toLowerCase()),
