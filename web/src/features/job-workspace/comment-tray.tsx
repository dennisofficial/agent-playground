...
export function CommentTray() {
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-[14px] border border-border bg-surface" style={{ boxShadow: '0 1px 2px rgba(20,18,12,.05)' }}>
      <div className="flex items-center gap-2 px-3 py-[9px] pl-[13px]">
        ...
        <button type="button" onClick={clearComments} className="rounded-md px-2.5 py-[3px] text-[11px] font-medium text-dim">
          Clear all
        </button>
      </div>
      {[...byFile.entries()].map(([file, items]) => (
        <div key={file}>
          ...
          {items.map((c) => (
            <div key={c.id} className="flex items-start gap-2.5 border-t border-hair px-[11px] py-2 pl-[13px] first:border-t-0">
              <span className="w-0.5 flex-none self-stretch rounded-full bg-accent-line" />
              <div className="min-w-0 flex-1">
                <div className="truncate text-[12.5px] italic leading-snug text-dim">&ldquo;{c.quote}&rdquo;</div>
                <div className={`truncate text-[12.5px] leading-snug ${c.note ? 'text-dim' : 'text-faint'}`}>
