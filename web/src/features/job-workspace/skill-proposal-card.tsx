...
const HEAD: Record<WebSkillProposalCard["mode"], { icon: typeof BookOpen; label: string; cta: string; pending: string }> = {
  install: { icon: Download, label: "Install a maintained skill", cta: "Approve & install", pending: "Installing…" },
  create: { icon: BookOpen, label: "New skill", cta: "Approve & add", pending: "Adding…" },
  remove: { icon: Trash2, label: "Remove a skill", cta: "Approve removal", pending: "Removing…" },
};
...
export function SkillProposalCard({
  ...
  if (card.approved_at != null) {
    const verb = card.mode === "remove" ? "removed" : card.mode === "install" ? "installed" : "added";
    return (
    ...
    <div className="anim-pop self-stretch overflow-hidden rounded-lg border border-border bg-surface">
      ...
      <div className="flex flex-col gap-2.5 px-4 py-3">
        ...
        {card.mode === "remove" ? (
          <div className="rounded-md border border-border bg-surface-2 px-3 py-2">
            <span className="font-mono text-[12.5px] font-semibold text-text">{card.name}</span>
            <p className="mt-0.5 text-[12px] text-dim">This skill will be deleted from every future build.</p>
          </div>
        ) : null}
