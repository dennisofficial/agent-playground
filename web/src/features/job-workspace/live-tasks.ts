...
function applyCall(byId: Map<string, TaskItem>, name: string, input: Record<string, unknown>, result: unknown): void {
  ...
  const extras = (prev?: TaskItem): Pick<TaskItem, 'description' | 'activeForm'> => ({
    ...(isStr(input.description) ? { description: input.description } : prev?.description ? { description: prev.description } : {}),
    ...(isStr(input.activeForm) ? { activeForm: input.activeForm } : prev?.activeForm ? { activeForm: prev.activeForm } : {}),
  });
  ...
  if (name === 'taskcreate') {
    ...
    const subject = isStr(input.subject) ? input.subject : isStr(input.description) ? input.description : id;
