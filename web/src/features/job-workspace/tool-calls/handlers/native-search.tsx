...
function SearchBody({ tool }: { tool: ToolItem }) {
  ...
  if (!body)
    return <TerminalBlock body={tool.isError ? "(error)" : "(no output)"} />;
  ...
  if (name === "read") {
    return (
      <ReadFileView
        result={body}
        lang={langFromPath(str(inp.file_path ?? inp.path ?? inp.notebook_path))}
      />
    );
  }
  if (
    name === "grep" &&
    str(inp.output_mode) === "content" &&
    isNumberedGrep(body)
  ) {
...
export const nativeSearchHandler: ToolHandler = {
  ...
  describe: (tool): ToolDescriptor => {
    const inp = asRecord(tool.input);
    const name = tool.name.toLowerCase();
    const errBadge: ToolDescriptor["badge"] = tool.isError
      ? { kind: "error" }
      : null;
    ...
    if (name === "read") {
      ...
      const badge: ToolDescriptor["badge"] = tool.isError
        ? { kind: "error" }
        : n
          ? { kind: "lines", n }
          : null;
