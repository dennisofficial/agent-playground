 ...
 */
export function McpProposalCard({ card, jobRef }: { card: WebMcpProposalCard; jobRef: JobRef }) {
  ...
  const statusOf = (name: string) =>
    mcp?.servers.find((s) => s.scope === scopeOf && s.name === name);
    ...
    <div className="anim-pop self-stretch overflow-hidden rounded-lg border border-border bg-surface">
      ...
      <div className="flex flex-col gap-2.5 px-4 py-3">
        ...
        <p className="text-[11.5px] leading-snug text-dim">
          Approving registers {card.servers.length === 1 ? 'this server' : 'these servers'} on this
          repo. Servers marked <span className="text-amber">needs secret</span> then ask you for a
          credential through a secure field — the value never appears in the conversation.
          {oauthServers.length > 0 ? (
            <>
              {' '}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {approve.isError ? (
              <span className="text-[11.5px] text-red">
                Could not register the servers. Try again.
              </span>
            ) : null}
            {oauth.result ? (
