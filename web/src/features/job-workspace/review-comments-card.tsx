 ...
 */
export function ReviewCommentsCardView({ card, time }: { card: WebReviewCommentsCard; time?: string }) {
  ...
  return (
    <div className="flex flex-col items-end gap-1">
        ...
        <div className="flex flex-col gap-2">
          {[...byFile.entries()].map(([file, items]) => (
            <div key={file} className="flex flex-col gap-2">
              {byFile.size > 1 ? (
                <div className="font-mono text-[9px] font-semibold uppercase tracking-[0.06em] text-accent-2">{file}</div>
              ) : null}
              {items.map((it, i) => (
      ...
      {card.message ? <UserBubble text={card.message} time={time} /> : <MessageTime iso={time} tone="user" align="right" />}
    </div>
  );
}
