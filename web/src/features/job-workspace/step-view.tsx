    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-hidden p-4">
        {meta.kind === "server" ? (
          ...
          <WebPortView url={meta.url ?? ""} app={meta.app ?? "billing"} />
        )}
      </div>
      <PortMockFootnote />
    </div>
...
function WebPortView({ url, app }: { url: string; app: "billing" | "admin" }) {
  return (
    <div
      className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-border-2 bg-panel"
      style={{ boxShadow: "var(--shadow-card)" }}
    >
      <div
        className="flex h-8 shrink-0 items-center gap-1.5 border-b border-border px-3"
      ...
      >
        ...
        />
        <span className="ml-2 flex-1 truncate rounded-md bg-panel px-3 py-1 font-mono text-[9px] text-dim">
          {url}
        </span>
      </div>
...
function MockBillingApp() {
  ...
  return (
    <div className="px-8 py-7">
      ...
      <div
        className="mt-5 flex items-center justify-between rounded-xl border border-border px-4 py-4"
        style={{ background: "var(--surface-2)" }}
      >
        <div>
          <div className="text-[14px] font-semibold text-text">Pro plan</div>
          <div className="text-[11px] text-dim">
            Renews Aug 1 · billed monthly
          </div>
        </div>
      ...
      <div className="mb-2 mt-6 font-mono text-[10px] tracking-[0.08em] text-faint">
        RECENT INVOICES
      </div>
      <div className="overflow-hidden rounded-xl border border-border">
        {invoices.map((inv, i) => (
          <div
            key={inv.date}
            className={cn(
              "flex items-center px-4 py-3 text-[12px]",
...
function MockAdminApp() {
  ...
  return (
    <div className="px-7 py-6">
      <div className="mb-5 text-[17px] font-bold text-text">
        Admin · Billing ops
      </div>
      <div className="mb-6 flex gap-3">
        {stats.map((s) => (
          ...
          >
            ...
            <div
              className={cn(
                "mt-1 text-[19px] font-bold",
                s.tone === "red" ? "text-red" : "text-text",
              )}
            >
              {s.value}
            </div>
      ...
      <div className="mb-2 font-mono text-[10px] tracking-[0.08em] text-faint">
      ...
      <div className="overflow-hidden rounded-xl border border-border">
        {events.map((e, i) => (
          <div
            key={e.name}
            className={cn(
              "flex items-center px-4 py-2.5 text-[11px]",
              i > 0 && "border-t border-border",
            )}
          >
            <span className="flex-1 font-mono text-[10px] text-blue">
              {e.name}
            </span>
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-border-2 bg-panel">
      <div className="flex shrink-0 items-center gap-2.5 border-b border-border px-4 py-3">
        ...
        <span className="font-mono text-[9.5px] text-dim">
          :8080 · node · uptime 2h 14m
        </span>
        <span className="flex-1" />
        <span className="font-mono text-[9px] text-faint">142 req/min</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto py-3">
        <div className="px-5 pb-2 font-mono text-[8px] tracking-[0.1em] text-faint">
          REQUEST LOG · LIVE
        </div>
        {log.map((r, i) => (
          <div
            key={i}
            className="flex items-center gap-3 whitespace-pre px-5 py-1 font-mono text-[10.5px]"
          >
            <span className="text-faint">{r.time}</span>
            <span
              className="w-9"
              style={{
                color: r.method === "POST" ? "var(--blue)" : "var(--green)",
              }}
            >
              {r.method}
            </span>
            ...
            <span
              style={{
                color: r.status === "200" ? "var(--green)" : "var(--red)",
              }}
            >
              {r.status}
            </span>
