...
/**
 * The live-stream lane a build THREAD streams on — STABLE per thread (parallel to the planner thread's
 * fixed `main` lane), so the web subscribes by thread identity. A thread's durable transcript is filtered
 * by the message's own `threadId` field, so this lane string is used only for the live turn subscription.
 */
export const threadLane = (threadId: string): string => `thread:${threadId}`;

/**
 * The opening "what was asked" block on a build THREAD (Leg) lane — the instruction the engine received
 * (goal + locked decisions + per-thread briefs). It's a long generated markdown DOCUMENT, not an operator
 ...
 * the planner's own turn) so prompt iteration isn't blind. Collapsible; `defaultOpen` is true on the agent
