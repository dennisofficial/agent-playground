...
function useApprovePlan(jobRef: JobRef, value: string) {
  ...
  return { submit, pending: approve.isPending, approved: approve.isSuccess, error: approve.isError };
}
...
export function NavigatorApprovalCallout({ jobRef, value }: { jobRef: JobRef; value: string }) {
  ...
  return (
    <div
      ...
      style={{ borderColor: 'var(--accent-line)', background: 'var(--accent-soft)', padding: '10px 11px' }}
    >
      <div className="flex items-center gap-1.5">
        <ClipboardCheck size={11} strokeWidth={2} style={{ color: 'var(--accent)' }} className="shrink-0" />
...
export function PersistentApprovalBar({
  ...
  return (
    <div
      ...
      style={{ borderColor: 'var(--accent-line)', background: 'var(--accent-soft)', padding: '10px 16px' }}
    >
      <div
      ...
      <div className="min-w-0 flex-1">
        <div className="text-[11.5px] font-bold text-text">
          {error ? 'Couldn’t approve — try again' : approved ? 'Plan approved' : 'Plan awaiting your approval'}
        </div>
        <div className="mt-px text-[10px] text-dim">
          {specCount} spec{specCount === 1 ? '' : 's'} · {stepCount} step{stepCount === 1 ? '' : 's'} · approve from
          anywhere in this pane
        </div>
      </div>
      <ApproveButton
        onClick={submit}
        ...
        style={{ borderRadius: '8px', padding: '8px 15px', border: '1px solid var(--green)' }}
        iconSize={13}
      />
    </div>
  );
...
function ApproveButton({
  ...
  return (
    <button
      ...
      style={{ background: 'var(--green)', boxShadow: '0 2px 8px var(--green-soft)', ...style }}
    >
      {pending ? (
        <>
