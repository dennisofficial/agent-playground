...
describe("contextConvoNodeForHref", () => {
  it("maps absolute /context bucket hrefs to their node id", () => {
    ...
    expect(
      contextConvoNodeForHref("/context/evidence/010-backend/RESULTS.md"),
    ).toBe("evidence:010-backend/RESULTS.md");
  });
  ...
  it("strips query and hash before resolving", () => {
    ...
    expect(
      contextConvoNodeForHref("/context/evidence/010-backend/run.log?v=2#tail"),
    ).toBe("evidence:010-backend/run.log");
  });
  ...
  it("resolves an evidence: node id (fourth bucket, self-handling like the others)", () => {
    expect(resolveNode("evidence:010-backend/RESULTS.md", null, false)).toBe(
      "found",
    );
  });
});
