...
export function JobMenu({
  ...
  const pickableSections = useMemo(() => {
    const pickable = allJobs.filter(
      (t) => t.org.id === jobRef.orgId && t.repo.id === jobRef.repoId && t.id !== jobRef.jobId,
    );
      ...
      {open ? (
        ...
        >
            ...
            <button
              ...
              onClick={() =>
                unblock.mutate(undefined, {
                  ...
                  onError: (err) =>
                    showToast(mutationErrorMessage(err, "Couldn't unblock this job.")),
                })
              }
          ...
          <button
            type="button"
            disabled={!canBlock}
            title={canBlock ? undefined : "Can't block a job that's building or finished"}
          ...
          {picking && canBlock ? (
            <div className="max-h-60 overflow-y-auto border-t border-border py-1">
              {pickableSections.length === 0 ? (
                <div className="px-3 py-1.5 text-[11px] text-faint">No other jobs on this repo</div>
              ) : (
                      ...
                      <button
                        ...
                        onClick={() =>
                          addDependency.mutate(t.id, {
                            ...
                            onError: (err) =>
                              showToast(mutationErrorMessage(err, "Couldn't block that job.")),
                          })
                        }
          ...
          {onDelete ? (
            <button
              type="button"
              disabled={deleting || deleteReady === false || status === 'archived'}
              onClick={() => {
                if (hasOpenPr) {
            ...
            >
              <Archive size={13} />
              {deleting ? 'Archiving…' : confirm ? 'Click again to confirm' : 'Archive job'}
            </button>
          ) : null}
        </div>
      ) : null}
