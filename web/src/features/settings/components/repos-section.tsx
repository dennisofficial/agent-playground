      ...
      <div className="flex items-start gap-3.5">
        ...
        {canManage && repos.length > 0 && !formOpen ? (
          <button
            ...
            style={{ background: 'var(--accent)', boxShadow: '0 4px 14px var(--accent-soft)' }}
          >
        ...
        <>
          {isMember ? (
            <div className="mb-3 flex items-center gap-1.5 text-[11.5px] text-faint">
              <Lock size={13} />
              Read-only — only owners can connect, re-validate, edit, or disconnect repos.
            </div>
          ) : null}
  ...
  return (
    ...
    >
      {ok ? (
        ...
        <AlertCircle size={15} strokeWidth={2} style={{ color: 'var(--red)' }} />
      )}
      <span className="text-[12.5px] font-medium" style={{ color: ok ? 'var(--green)' : 'var(--red)' }}>
        {flash.text}
      </span>
    </div>
  );
...
function Sk({ w, h, radius = '5px' }: { w: string; h: string; radius?: string }) {
  return (
    <span
      className="block animate-pulse"
      style={{ width: w, height: h, borderRadius: radius, background: 'var(--surface-2)' }}
    />
  );
}
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div
      ...
      style={{ background: RED_SOFT, border: `1px solid color-mix(in srgb, var(--red) 30%, transparent)` }}
    >
      <span
        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[9px] bg-surface"
        style={{ border: `1px solid color-mix(in srgb, var(--red) 38%, transparent)`, color: 'var(--red)' }}
      >
        <AlertCircle size={17} />
      </span>
      <div className="flex-1">
        <div className="text-[13.5px] font-semibold" style={{ color: 'var(--red)' }}>
  ...
  return (
    ...
    >
      <span
        ...
        style={
          withToken
            ? { color: 'var(--accent)', boxShadow: '0 4px 14px var(--accent-soft)' }
            : { color: 'var(--faint)' }
        }
      >
...
function NoTokenState({ onOpenCredentials }: { onOpenCredentials: () => void }) {
  return (
    <>
      ...
      >
        ...
        <div className="flex-1">
          <div className="text-[12.5px] font-semibold text-text">A GitHub token is required</div>
      ...
      >
        ...
        <label className="mb-2 block text-[12px] font-medium text-dim">Repo URL</label>
...
function ConnectForm({
  ...
  async function submit() {
    ...
    if (!GITHUB_URL.test(raw)) {
      setError('Not an HTTPS GitHub URL — expected https://github.com/owner/repo');
      return;
    }
    setError('');
    try {
  ...
  return (
    ...
    >
      <div className="mb-3.5 flex items-center gap-2.5">
        ...
        <div className="flex-1 text-[13.5px] font-semibold text-text">Connect a repository</div>
        {validating ? (
          <span
      ...
      <div className="mt-3 flex gap-3">
        ...
        <div className="w-40">
          <label className="mb-2 block text-[12px] font-medium text-dim">Base branch</label>
          <input
            value={branch}
      ...
      {error ? (
        <div className="mt-2.5 flex items-center gap-1.5 text-[11.5px]" style={{ color: 'var(--red)' }}>
          <AlertCircle size={13} />
          {error}
        </div>
      ) : null}
      ...
      <p className="mt-2.5 text-[11px] leading-relaxed text-faint">
        Uses the org GitHub token. A repo your token can’t reach is still saved — you can re-validate it later.
      </p>
...
function RepoRow({
  ...
  const pill = revalidating
    ? { text: 'Re-validating…', color: 'var(--accent)', bg: 'var(--accent-soft)', border: 'var(--accent-line)' }
    : repo.accessOk
      ? { text: 'Connected', color: 'var(--green)', bg: 'var(--green-soft)', border: GREEN_BORDER }
      : { text: 'Access failed', color: 'var(--red)', bg: RED_SOFT, border: RED_BORDER };
    ...
    <div className="flex items-start gap-4 px-4 py-[15px]">
      ...
      <div className="flex shrink-0 flex-col items-end gap-2.5">
        ...
        {!revalidating && checked ? (
          <span className="font-mono text-[9.5px] text-faint">checked {checked}</span>
        ) : null}
  ...
  async function save() {
    ...
    try {
      await update.mutateAsync({
        repoId: repo.id,
        body: { name: name.trim() || repo.name, defaultBranch: branch.trim() || repo.defaultBranch },
      });
      onClose();
    } catch (e) {
...
function DisconnectDialog({
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(10,12,16,.5)', backdropFilter: 'blur(3px)', paddingTop: 130 }}
    >
      <div
        ...
        <div className="px-5 pb-[18px] pt-5">
          <div className="mb-3 flex items-center gap-2.5">
            <span
              ...
              style={{ background: RED_SOFT, border: `1px solid ${RED_BORDER}`, color: 'var(--red)' }}
            >
              <Power size={17} />
            </span>
            <div className="font-disp text-[16px] font-semibold text-text">Disconnect {repo.name}?</div>
          </div>
          {hasThreads ? (
            <div className="text-[12.5px] leading-relaxed" style={{ color: 'var(--red)' }}>
              This permanently deletes <strong>{threadLabel}</strong> and all their work — feature
              branches, sandboxes, messages, and history. This can’t be undone.
            </div>
          ) : (
          ...
          <div
            className="mt-3 flex items-center gap-2 rounded-md border border-border bg-surface-2 px-3 py-2.5 font-mono text-[11px] text-dim"
          >
            <GitBranch size={13} />
            <span className="truncate">{repo.gitUrl}</span>
          </div>
          {error ? (
            <div className="mt-3 flex items-center gap-1.5 text-[11.5px]" style={{ color: 'var(--red)' }}>
              <AlertCircle size={13} />
              {error}
            </div>
          ) : null}
