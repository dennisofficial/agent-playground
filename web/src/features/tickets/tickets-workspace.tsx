...
type ModalState = { mode: 'create' } | { mode: 'edit'; ticket: TicketListRow } | null;
...
export function TicketsWorkspace({ orgId, repoId }: { orgId: string; repoId: string }) {
  ...
  const boardCount = useMemo(
    () => tickets.filter((t) => (BOARD_COLUMNS as string[]).includes(t.status)).length,
    [tickets],
  );
  const backlogCount = useMemo(() => tickets.filter((t) => t.status === 'backlog').length, [tickets]);
  const nextNumber = useMemo(() => tickets.reduce((a, t) => Math.max(a, t.number), 0) + 1, [tickets]);
  const drawerRow = drawerId ? tickets.find((t) => t.id === drawerId) ?? null : null;
  ...
  function submitForm(value: TicketFormValue) {
    ...
    if (modal?.mode === 'edit') {
      update.mutate({ ticketId: modal.ticket.id, body }, { onSuccess: () => setModal(null) });
    } else {
    ...
    <div className="flex h-full min-h-0 flex-col bg-bg">
      ...
      <div className="flex flex-none items-center gap-3.5 px-[26px] pt-4">
        <div className="flex min-w-0 items-center gap-2">
          <span className="h-[9px] w-[9px] flex-none rounded-[2px]" style={{ background: 'var(--accent)' }} />
          <span className="font-mono text-[12px] text-dim">{meta?.orgName ?? '—'}</span>
          <span className="text-border-2">/</span>
          <span className="font-mono text-[12px] font-semibold text-text">{meta?.repo.name ?? repoId}</span>
        </div>
      </div>
      ...
      <div className="flex flex-none flex-wrap items-center gap-3 px-[26px] pb-3 pt-3.5">
        ...
        >
          <span className="pulse-dot h-1.5 w-1.5 rounded-full" style={{ background: 'var(--green)' }} />
        ...
        <button
          ...
          style={{ background: 'linear-gradient(145deg, var(--accent), var(--accent-2))', boxShadow: '0 4px 12px var(--accent-soft)' }}
        >
      ...
      {/* body */}
      {isLoading ? (
        <div className="flex flex-1 items-center justify-center text-[12px] text-faint">Loading tickets…</div>
      ) : view === 'board' ? (
        <Board
          tickets={tickets}
          q={q}
      ...
      {drawerRow ? (
        <TicketDetailModal
          ...
          onDelete={() => del.mutate(drawerRow.id, { onSuccess: () => setDrawerId(null) })}
            ...
            <div key={col} className="flex w-[270px] flex-none flex-col">
              <div className="flex flex-none items-center gap-2 px-1 pb-2.5 pt-2">
                <span
                  ...
                  style={{ background: statusColor(col), boxShadow: col === 'in_progress' ? '0 0 7px var(--accent)' : 'none' }}
                />
                <span className="text-[12px] font-semibold text-text">{STATUS_LABEL[col]}</span>
                <span className="font-mono text-[10px] text-faint">{all.length}</span>
                <div className="flex-1" />
                {threadDriven ? (
              ...
              <div
                ...
                style={col === 'done' ? { background: 'color-mix(in srgb, var(--surface-2) 55%, transparent)' } : undefined}
              >
                {cards.map((t) => (
...
function BoardCard({ t, onOpen }: { t: TicketListRow; onOpen: () => void }) {
  ...
  return (
    <button
      ...
      style={{ opacity: dim ? 0.6 : 1, boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}
    >
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex-1" />
        {t.priority ? (
          <span className="flex items-center gap-1 font-mono text-[8.5px] font-semibold" style={{ color: priorityColor(t.priority) }}>
            <span className="h-1.5 w-1.5 rounded-[2px]" style={{ background: priorityColor(t.priority) }} />
            {cap(t.priority)}
          </span>
        ) : null}
      </div>
      <div className="text-[13px] font-semibold leading-snug tracking-[-0.005em] text-text">{t.title}</div>
      {showMeta ? (
        <div className="flex flex-wrap items-center gap-1.5">
          {t.blocked ? (
            ...
            >
              <Lock size={9} /> blocked by {t.blockedBy ? `#${t.blockedBy}` : '—'}
            </span>
          ) : null}
          {t.linkedThreadId ? (
            <span
      ...
      <div className="mt-px flex items-center gap-1.5">
        <Marker atlas={atlas} />
        <span className="font-mono text-[8.5px] text-faint">{atlas ? 'Atlas' : 'You'}</span>
        <div className="flex-1" />
        <span className="font-mono text-[8.5px] text-faint">{timeAgo(t.updatedAt)}</span>
      </div>
    </button>
  );
}
...
function Backlog({
  ...
  const rows = backlog
    ...
    .sort((a, b) => (PRIORITY_RANK[a.priority ?? 'low'] ?? 9) - (PRIORITY_RANK[b.priority ?? 'low'] ?? 9));
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-[26px] pb-6">
      <div className="max-w-[880px]">
        <div className="flex items-center gap-2 px-0.5 pb-2.5 pt-1">
          <span className="font-mono text-[9px] tracking-[0.14em] text-faint">TRIAGE · {rows.length}</span>
          <div className="h-px flex-1" style={{ background: 'var(--border)' }} />
          <span className="text-[10px] text-faint">Promote to start work, or let Atlas commit it to the board.</span>
        </div>
        ...
        {cancelled.length > 0 ? (
          <>
            ...
            >
              ...
              />
              <span className="font-mono text-[9px] tracking-[0.14em] text-faint">CANCELLED · {cancelled.length}</span>
              <div className="h-px flex-1" style={{ background: 'var(--border)' }} />
            </button>
            {showCancelled ? (
              <div className="mt-1 flex flex-col gap-1.5">
                {cancelled.map((t) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-[10px] text-faint">#{t.number}</span>
                    <span className="min-w-0 flex-1 truncate text-[12.5px] font-medium text-dim line-through">{t.title}</span>
                    <span className="font-mono text-[8.5px] text-faint">cancelled · {timeAgo(t.updatedAt)}</span>
                  </button>
                ))}
              </div>
            ) : null}
  ...
  return (
    ...
    >
      <div className="flex w-[42px] flex-none flex-col items-center gap-1.5 pt-0.5">
        {t.priority ? (
          <span className="grid h-[18px] w-[18px] place-items-center rounded-[5px]" style={{ background: priorityColor(t.priority) }} title={cap(t.priority)}>
            <span className="h-1.5 w-1.5 rounded-[2px] bg-white" />
          </span>
        ) : (
          <span className="h-[18px] w-[18px] rounded-[5px] border-[1.4px] border-dashed border-border-2" title="No priority" />
        )}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] text-faint">#{t.number}</span>
          {t.kind ? (
            <span className="rounded-[3px] border border-border-2 px-1 py-px font-mono text-[8px] font-bold tracking-[0.07em]" style={{ color: kindColor(t.kind) }}>
              {t.kind.toUpperCase()}
            </span>
          ) : null}
          {t.blocked ? (
        ...
        <div className="mt-1.5 text-[13.5px] font-semibold leading-snug tracking-[-0.005em] text-text">{t.title}</div>
        {body ? (
          <div className="mt-1 truncate text-[11.5px] leading-relaxed text-dim">{body}</div>
        ) : (
          <div className="mt-1 text-[11px] italic text-faint">No description</div>
        )}
        <div className="mt-2 flex items-center gap-1.5">
          ...
          {t.origin?.threadTitle ? (
            <span className="font-mono text-[8.5px] text-faint">· from {t.origin.threadTitle}</span>
          ) : null}
        </div>
      </div>
...
function Segmented<T extends string>({
  ...
  return (
    <div className="flex items-center gap-0.5 rounded-lg border border-border p-[3px]" style={{ background: 'var(--surface-2)' }}>
      {options.map((o) => (
        <button
          key={o.v}
          type="button"
          ...
          style={value === o.v ? { background: 'var(--surface)', boxShadow: '0 1px 2px rgba(0,0,0,0.06)' } : undefined}
        >
          {o.label}
          {o.count !== undefined ? <span className="font-mono text-[9px] text-faint">{o.count}</span> : null}
        </button>
      ))}
    </div>
  );
...
function Marker({ atlas }: { atlas: boolean }) {
  return atlas ? (
    <span className="h-[9px] w-[9px] flex-none rounded-[1.5px] border-[1.3px] border-accent" style={{ transform: 'rotate(45deg)' }} />
  ) : (
    <span className="h-[9px] w-[9px] flex-none rounded-full border border-border-2" style={{ background: 'var(--surface-3)' }} />
  );
}
...
function EmptyIcon({ lines }: { lines?: boolean }) {
  return (
    ...
    >
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--faint)" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
        {lines ? (
