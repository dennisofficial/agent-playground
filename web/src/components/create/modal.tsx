 ...
 */
export function Modal({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
      ...
      >
        <div className="flex items-start justify-between border-b border-border px-5 py-4">
          <div>
            ...
            {subtitle ? <p className="mt-0.5 text-[12.5px] text-dim">{subtitle}</p> : null}
          </div>
          <button
            type="button"
            onClick={() => router.back()}
